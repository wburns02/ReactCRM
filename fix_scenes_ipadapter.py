#!/usr/bin/env python3
"""
Fix Alien Abduction Scenes with IPAdapter (Style-Based)
For stylized/video game character references that don't work with FaceID
"""
...
SCENES_TO_FIX = [
    ...
    {
        "scene": 1,
        "prompt": "Photorealistic cinematic still frame, 35mm film look, a man standing beside an old pickup truck on a dirt road at night, tall corn stalks surrounding him, looking up at the sky with curiosity, dark atmospheric lighting with subtle moonlight, slight lens flare from truck headlights, professional cinematography, film grain, moody dark blue color grading, ultra detailed, 8K, the man has olive/tan skin, dark brown shaggy hair, mustache, wearing brown jacket",
    ...
    {
        "scene": 3,
        "prompt": "Photorealistic cinematic still frame, 35mm film look, a massive dark metallic UFO hovering above a cornfield at night, intense bright lights emanating from underneath the craft, a man looking up in shock and awe, atmospheric perspective, volumetric fog and light beams, professional cinematography, film grain, dark atmospheric lighting, dramatic scale showing craft dwarfing the human figure, 8K ultra detailed, moody dark blue color grading, the man has olive/tan skin, dark brown shaggy hair, mustache, wearing brown jacket",
...
def create_ipadapter_scene_workflow(scene_prompt, output_prefix, seed):
    """Create workflow with IPAdapter for character-consistent scene generation"""
    ...
    return {
        # Load character reference image (single best one)
        "1": {
...
def main():
    print("=" * 60)
    print("FIXING ALIEN SCENES WITH IPADAPTER")
    print("Style-based character consistency")
    print("=" * 60)
    ...
    print("\nUsing chel1.png as reference (copy to input if needed)")
    ...
    for scene_info in SCENES_TO_FIX:
        ...
        workflow = create_ipadapter_scene_workflow(
            scene_info['prompt'],
            scene_info['output'],
            scene_info['seed']
        )
