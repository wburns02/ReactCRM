...
const GPS_KEYS = {
  ...
  location: (technicianId: number) => [...GPS_KEYS.locations(), technicianId] as const,
  allLocations: () => [...GPS_KEYS.locations(), 'all'] as const,
  history: (technicianId: number, date?: string) => [...GPS_KEYS.all, 'history', technicianId, date] as const,
  ...
  trackingLinks: (workOrderId: number) => [...GPS_KEYS.all, 'tracking-links', workOrderId] as const,
  publicTracking: (token: string) => [...GPS_KEYS.all, 'public-tracking', token] as const,
  dispatchMap: () => [...GPS_KEYS.all, 'dispatch-map'] as const,
  config: () => [...GPS_KEYS.all, 'config'] as const,
  technicianConfig: (technicianId: number) => [...GPS_KEYS.config(), technicianId] as const,
};
...
export function useTechnicianLocation(technicianId: number) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<TechnicianLocation>(`/gps/location/${technicianId}`);
...
export function useAllTechnicianLocations() {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<AllTechniciansLocationResponse>('/gps/locations');
      return response.data;
    },
    refetchInterval: 10000, // Refresh every 10 seconds
    staleTime: 5000,
...
export function useLocationHistory(technicianId: number, date?: string, workOrderId?: number) {
  return useQuery({
...
export function useUpdateLocation() {
  ...
  return useMutation({
    mutationFn: async (location: LocationUpdate) => {
      const response = await apiClient.post<TechnicianLocation>('/gps/location', location);
      return response.data;
    },
    onSuccess: (data) => {
...
export function useUpdateLocationBatch() {
  ...
  return useMutation({
    mutationFn: async (locations: LocationUpdate[]) => {
      const response = await apiClient.post<{ processed: number; total: number; success: boolean }>(
        '/gps/location/batch',
        { locations }
      );
      return response.data;
...
export function useETA(workOrderId: number, enabled: boolean = true) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<ETAResponse>(`/gps/eta/${workOrderId}`);
      return response.data;
    },
...
export function useRecalculateETA() {
  ...
  return useMutation({
    mutationFn: async (workOrderId: number) => {
      const response = await apiClient.get<ETAResponse>(`/gps/eta/${workOrderId}?recalculate=true`);
      return response.data;
    },
    onSuccess: (data) => {
...
export function useGeofences(type?: string, isActive?: boolean) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await apiClient.get<Geofence[]>(`/gps/geofences?${params.toString()}`);
      return response.data;
    },
  });
}
...
export function useGeofenceEvents(filters?: {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (filters?.geofence_id) params.append('geofence_id', filters.geofence_id.toString());
      if (filters?.start_date) params.append('start_date', filters.start_date);
      if (filters?.end_date) params.append('end_date', filters.end_date);
      if (filters?.limit) params.append('limit', filters.limit.toString());
      const response = await apiClient.get<GeofenceEvent[]>(`/gps/geofences/events?${params.toString()}`);
      return response.data;
    },
  });
}
...
export function useUpdateGeofence() {
  ...
  return useMutation({
    mutationFn: async ({ id, data }: { id: number; data: Partial<GeofenceCreate> }) => {
      const response = await apiClient.patch<Geofence>(`/gps/geofences/${id}`, data);
      return response.data;
    },
    onSuccess: (data) => {
...
export function useTrackingLinks(workOrderId: number) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<TrackingLink[]>(`/gps/tracking-links/${workOrderId}`);
      return response.data;
    },
    enabled: !!workOrderId,
  });
...
export function usePublicTracking(token: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<PublicTrackingInfo>(`/gps/track/${token}`);
      return response.data;
    },
    enabled: !!token,
...
export function useCreateTrackingLink() {
  ...
  return useMutation({
    mutationFn: async (data: TrackingLinkCreate) => {
      const response = await apiClient.post<TrackingLink>('/gps/tracking-links', data);
      return response.data;
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: GPS_KEYS.trackingLinks(data.work_order_id) });
    },
  });
}
...
export function useTechnicianGPSConfig(technicianId: number) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.get<GPSConfig>(`/gps/config/${technicianId}`);
      return response.data;
    },
    enabled: !!technicianId,
  });
