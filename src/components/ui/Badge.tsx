...
export interface BadgeProps extends HTMLAttributes<HTMLSpanElement> {
  variant?: 'default' | 'success' | 'warning' | 'danger' | 'info' | 'stage' | 'primary' | 'secondary' | 'outline' | 'destructive';
 ...
 */
export function Badge({ className, variant = 'default', stage, size, children, ...props }: BadgeProps) {
    ...
    <span
      className={cn(
        'inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold',
        {
          ...
          'bg-danger-light text-danger': variant === 'danger' || variant === 'destructive',
