...
export function PhotoAnalyzer({
  ...
  const { isAnalyzing, result, error, preview, analyzePhoto, reset } =
    usePhotoAnalysis();
        ...
        {!isAIAvailable && (
          <div className="flex items-center gap-2 p-3 bg-warning/10 border border-warning/30 rounded-lg">
            ...
            <p className="text-xs text-warning">
              Local AI services are currently unavailable. Photo analysis will
              be queued.
            </p>
          </div>
        )}
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-3 bg-bg-muted rounded-lg">
                  <div>
                    ...
                    <p
                      className={`text-lg font-bold ${getConditionColor(result.condition_rating)}`}
                    >
                      {result.condition_rating}/10
                    </p>
                  </div>
                  <Badge
                    variant={getConditionBadgeVariant(result.condition_rating)}
                  >
