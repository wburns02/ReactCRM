...
const validHistory = {
  ...
  response:
    "I assigned 5 jobs to available technicians based on skills and location.",
  suggestions_count: 5,
  executed: true,
...
describe("AI Dispatch API Contracts", () => {
  describe("aiDispatchActionSchema", () => {
    ...
    it("validates primary and secondary action types", () => {
      expect(
        aiDispatchActionSchema.safeParse({ ...validAction, type: "primary" })
          .success,
      ).toBe(true);
      expect(
        aiDispatchActionSchema.safeParse({ ...validAction, type: "secondary" })
          .success,
      ).toBe(true);
    });

    it("rejects invalid action type", () => {
      const result = aiDispatchActionSchema.safeParse({
        ...validAction,
        type: "invalid",
      });
  ...
  describe("aiDispatchSuggestionSchema", () => {
    ...
    it("validates all suggestion types", () => {
      ...
      types.forEach((type) => {
        const result = aiDispatchSuggestionSchema.safeParse({
          ...validSuggestion,
          type,
        });
        expect(result.success).toBe(true);
    ...
    it("validates confidence range 0-1", () => {
      expect(
        aiDispatchSuggestionSchema.safeParse({
          ...validSuggestion,
          confidence: 0,
        }).success,
      ).toBe(true);
  ...
  describe("aiDispatchSuggestionsResponseSchema", () => {
    ...
    it("validates empty suggestions array", () => {
      const result = aiDispatchSuggestionsResponseSchema.safeParse({
        suggestions: [],
      });
      expect(result.success).toBe(true);
    });
  ...
  describe("aiDispatchStatsSchema", () => {
    ...
    it("validates acceptance_rate boundaries", () => {
      expect(
        aiDispatchStatsSchema.safeParse({ ...validStats, acceptance_rate: 0 })
          .success,
      ).toBe(true);
      expect(
        aiDispatchStatsSchema.safeParse({ ...validStats, acceptance_rate: 1 })
          .success,
      ).toBe(true);
      expect(
        aiDispatchStatsSchema.safeParse({ ...validStats, acceptance_rate: 1.1 })
          .success,
      ).toBe(false);
    });
  });
  ...
  describe("aiDispatchHistorySchema", () => {
    ...
    it("validates executed false", () => {
      const result = aiDispatchHistorySchema.safeParse({
        ...validHistory,
        executed: false,
      });
      expect(result.success).toBe(true);
  ...
  describe("aiDispatchRequestSchema", () => {
    it("validates minimal request with just prompt", () => {
      const result = aiDispatchRequestSchema.safeParse({
        prompt: "Optimize routes",
      });
      expect(result.success).toBe(true);
    });
      ...
      const response = {
        assignments: [
          ...
          },
          {
            work_order_id: "wo-2",
            technician_id: 2,
            scheduled_time: "2026-01-02T10:00:00Z",
          },
        ],
        failures: [
          {
