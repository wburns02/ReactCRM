...
describe('useNotifications hooks', () => {
  ...
  describe('notificationKeys', () => {
    it('generates correct query keys', () => {
      ...
      expect(notificationKeys.subscriptions()).toEqual(['notifications', 'subscriptions']);
    });
  });
  ...
  describe('useNotifications', () => {
    it('fetches notifications list successfully', async () => {
      ...
      expect(result.current.data).toEqual({ notifications: [mockNotification], total: 1 });
    });
    ...
    it('returns default value when endpoint not implemented (404)', async () => {
      vi.mocked(withFallback).mockResolvedValue({ notifications: [], total: 0 });
  ...
  describe('useNotificationStats', () => {
    ...
    it('returns default stats on 404', async () => {
      vi.mocked(withFallback).mockResolvedValue({ total: 0, unread: 0, by_type: {} });
  ...
  describe('useNotificationPreferences', () => {
    it('fetches notification preferences successfully', async () => {
      ...
      const { result } = renderHookWithClient(() => useNotificationPreferences());
  ...
  describe('useUpdateNotificationPreferences', () => {
    it('updates preferences and invalidates query', async () => {
      vi.mocked(apiClient.put).mockResolvedValue({ data: { ...mockPreferences, sms_enabled: true } });

      const { result, queryClient } = renderHookWithClient(() => useUpdateNotificationPreferences());
      ...
      expect(apiClient.put).toHaveBeenCalledWith('/notifications/preferences', { sms_enabled: true });
  ...
  describe('usePushSubscriptions', () => {
    it('fetches push subscriptions successfully', async () => {
      vi.mocked(apiClient.get).mockResolvedValue({ data: { subscriptions: [mockSubscription] } });
  ...
  describe('useRegisterPushSubscription', () => {
    it('registers subscription and invalidates query', async () => {
      ...
      const { result, queryClient } = renderHookWithClient(() => useRegisterPushSubscription());
  ...
  describe('useUnregisterPushSubscription', () => {
    it('unregisters subscription and invalidates query', async () => {
      ...
      const { result, queryClient } = renderHookWithClient(() => useUnregisterPushSubscription());
      ...
      expect(apiClient.delete).toHaveBeenCalledWith('/notifications/push/subscriptions/sub-123');
      expect(invalidateSpy).toHaveBeenCalledWith({
        queryKey: notificationKeys.subscriptions(),
      });
    });
  ...
  describe('useMarkNotificationRead', () => {
    it('marks notification as read and invalidates all notification queries', async () => {
      ...
      const { result, queryClient } = renderHookWithClient(() => useMarkNotificationRead());
  ...
  describe('useMarkAllNotificationsRead', () => {
    it('marks all notifications as read and invalidates queries', async () => {
      ...
      const { result, queryClient } = renderHookWithClient(() => useMarkAllNotificationsRead());
  ...
  describe('useDeleteNotification', () => {
    it('deletes notification and invalidates queries', async () => {
      ...
      const { result, queryClient } = renderHookWithClient(() => useDeleteNotification());
  ...
  describe('useVapidPublicKey', () => {
    it('fetches VAPID public key', async () => {
      vi.mocked(apiClient.get).mockResolvedValue({ data: { public_key: 'BEAbc123...' } });
