...
describe('useRouteOptimization hooks', () => {
  ...
  describe('useOptimizeRoutes', () => {
    it('optimizes routes for a given date', async () => {
      vi.mocked(apiClient.post).mockResolvedValue({ data: mockOptimizationResponse });
      ...
      expect(apiClient.post).toHaveBeenCalledWith('/scheduling/optimize-routes', request);
  ...
  describe('useApplyOptimizedRoutes', () => {
    it('applies optimized routes to work orders', async () => {
      vi.mocked(apiClient.post).mockResolvedValue({ data: { success: true, updated_count: 5 } });
      ...
      expect(apiClient.post).toHaveBeenCalledWith('/scheduling/apply-optimized-routes', {
        routes: [mockTechnicianRoute],
      });
  ...
  describe('useRoutePreview', () => {
    it('fetches route preview for work orders', async () => {
      vi.mocked(apiClient.post).mockResolvedValue({ data: mockRoutePreviewResponse });
  ...
  describe('useDrivingDirections', () => {
    it('fetches driving directions between two points', async () => {
      vi.mocked(apiClient.post).mockResolvedValue({ data: mockDirectionsResponse });

      const { result } = renderHookWithClient(() => useDrivingDirections());

      result.current.mutate({
        origin: { lat: 30.25, lng: -97.75 },
        destination: { lat: 30.30, lng: -97.70 },
      });

      await waitFor(() => expect(result.current.isSuccess).toBe(true));

      expect(apiClient.post).toHaveBeenCalledWith('/scheduling/driving-directions', {
        origin: { lat: 30.25, lng: -97.75 },
        destination: { lat: 30.30, lng: -97.70 },
      });
  ...
  describe('useOptimizationHistory', () => {
    it('fetches optimization history for date range', async () => {
      vi.mocked(apiClient.get).mockResolvedValue({ data: mockOptimizationHistory });
      ...
      expect(apiClient.get).toHaveBeenCalledWith('/scheduling/optimization-history', {
