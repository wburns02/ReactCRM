...
describe("AI Assistant Integration", () => {
  ...
  describe.skip("Query Processor", () => {
    ...
    it("should extract entities from queries", () => {
      ...
      const query =
        "Show me customer John Smith's work order #WO123 scheduled for today";
  ...
  describe("Context Manager", () => {
    it("should aggregate context from multiple sources", async () => {
      ...
      const context = await contextManager.aggregateContext(
        "test_user",
        "test_session",
      );
    ...
    it("should cache context appropriately", async () => {
      ...
      // First call
      const context1 = await contextManager.aggregateContext(
        "test_user",
        "test_session",
      );

      // Second call (should use cache)
      const context2 = await contextManager.aggregateContext(
        "test_user",
        "test_session",
      );
  ...
  describe.skip("End-to-End AI Assistant", () => {
    it("should integrate all components correctly", async () => {
      ...
      expect(query.naturalLanguageQuery).toBe(
        "Show me customer activity summary",
      );
    });
  ...
  describe("Error Handling", () => {
    it("should provide meaningful error messages", async () => {
      ...
      await expect(adapter.query(invalidQuery, mockContext)).rejects.toThrow(
        "Unsupported ActivityAI query type",
      );
    });
    ...
    it("should handle adapter health check failures", async () => {
      ...
      adapter["performHealthCheck"] = vi
        .fn()
        .mockRejectedValue(new Error("Adapter offline"));
