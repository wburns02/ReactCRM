...
export class DispatchAIAdapter extends BaseAIAdapterImpl<
  DispatchQuery,
  DispatchResult
> {
  ...
  async query(
    ...
    context: AIContext,
  ): Promise<UnifiedAIResponse<DispatchResult>> {
    ...
    try {
      ...
      response.actionable_insights = this.generateDispatchInsights(
        result,
        context,
      );
  ...
  private generateRouteOptimization(
    ...
    return {
      ...
      schedule_analysis: {
        ...
        assigned_jobs: 22,
        unassigned_jobs: 2,
        utilization_by_tech: technicians.reduce(
          (acc, tech) => ({
            ...acc,
      ...
      alerts: [
        {
          ...
          suggested_action:
            "Consider rescheduling North Austin jobs to afternoon",
        },
      ],
    };
  }
      ...
      recommendations: [
        {
          ...
          description:
            "Consider adding an extra technician for the afternoon shift to handle overflow",
          impact: {
      ...
      alerts: [
        {
          ...
          type: "capacity_issue",
          message:
            "Afternoon shift at risk of not completing all scheduled jobs",
      ...
      recommendations: [
        {
          ...
          technician_name:
            context.domain.technicians?.[0]?.name || "Mike Johnson",
          work_order_id: "WO-1234",
          description:
            "Job WO-1234 likely to run 30 minutes over - consider notifying next customer",
          impact: {
      ...
      alerts: [
        {
          ...
          suggested_action:
            "Proactively notify customers about potential 15-30 minute delays",
        },
      ],
    };
  }
  ...
  private generateSchedulingSuggestions(
    ...
    return {
      recommendations: (params.work_order_ids || []).map((woId, index) => ({
        ...
        technician_name:
          technicians[index % technicians.length]?.name || "Available Tech",
        work_order_id: woId,
  ...
  private generateDispatchInsights(
    ...
    // Efficiency insight
    if (
      result.optimization_metrics.optimized_efficiency >
      result.optimization_metrics.current_efficiency
    ) {
      ...
      insights.push({
        ...
        estimated_impact: {
          type: "time_saved",
          amount:
            result.optimization_metrics.total_drive_time_current -
            result.optimization_metrics.total_drive_time_optimized,
          unit: "minutes",
        },
        estimatedValue: {
          type: "time_saved",
    ...
    if (result.schedule_analysis.unassigned_jobs > 0) {
      insights.push({
        ...
        priority:
          result.schedule_analysis.unassigned_jobs > 3 ? "high" : "medium",
        confidence: 1.0,
      });
    }
  ...
  private extractAffectedEntities(
    ...
    if (action.payload.technician_id) {
      entities.push({
        type: "technician",
        id: action.payload.technician_id as string,
      });
    }
    if (action.payload.work_order_id) {
      entities.push({
        type: "work_order",
        id: action.payload.work_order_id as string,
      });
    }
  ...
  private createErrorResponse(
    error: unknown,
    startTime: number,
  ): UnifiedAIResponse<DispatchResult> {
    return {
      domain: this.domain,
      operation: "error",
      result: {
