...
export class ActivityAIAdapter extends BaseAIAdapterImpl<
  ActivityQuery,
  ActivityResult
> {
  ...
  async query(
    ...
    context: AIContext,
  ): Promise<UnifiedAIResponse<ActivityResult>> {
    ...
    try {
      ...
      throw new Error(
        `ActivityAI query failed: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  }

  private async executeActivityQuery(
    request: ActivityQuery,
    context: AIContext,
  ...
  private async getActivitySummary(
    ...
    const customerId =
      request.customerId || this.extractCustomerFromContext(context);
    if (!customerId) {
  ...
  private async getRangeSummary(
    ...
    if (!customerId || !startDate || !endDate) {
      throw new Error(
        "Customer ID, start date, and end date required for range summary",
      );
    }
  ...
  private isDemoDataUsed(result: ActivityResult): boolean {
    ...
    return (
      result.summary.toLowerCase().includes("demo") ||
  ...
  private generateActionableInsights(
    ...
    if (result.sentiment === "negative") {
      insights.push({
        ...
        description:
          "Recent interactions show negative sentiment. Immediate attention recommended.",
        priority: "high",
        confidence: 0.85,
        estimated_impact: {
    ...
    if (result.interaction_quality < 6) {
      insights.push({
        ...
        description:
          "Communication quality could be improved for better customer experience.",
        priority: "medium",
        confidence: 0.75,
        estimated_impact: {
          type: "time_saved",
    ...
    if (result.action_items.length > 5) {
      insights.push({
        ...
        description:
          "Multiple action items identified. Consider prioritizing by impact.",
        priority: "medium",
        confidence: 0.8,
      });
    }
        ...
        requirements: [
          {
            ...
            satisfied: context.user.permissions.includes(
  ...
  private generateFollowUpQuestions(result: ActivityResult): string[] {
    ...
    if (result.customer_id) {
      questions.push(
        `What other customers have similar activity patterns to ${result.customer_id}?`,
      );
    ...
    if (result.topics.length > 0) {
      questions.push(
        `Show me more customers discussing ${result.topics[0]} topics`,
      );
    }

    if (result.sentiment === "negative") {
      questions.push(
        "What actions have been most effective for improving customer satisfaction?",
      );
    }

    questions.push(
      "How does this activity compare to the customer's historical engagement?",
    );
  ...
  private extractDemoActionItems(activities: Activity[]): string[] {
    ...
    if (
      activities.some((a) => a.description?.toLowerCase().includes("payment"))
    ) {
      actionItems.push("Follow up on payment status");
    }

    if (
      activities.some((a) => a.description?.toLowerCase().includes("service"))
    ) {
  ...
  private inferSentimentFromActivities(
    activities: Activity[],
  ): "positive" | "neutral" | "negative" {
    const descriptions = activities
      .map((a) => a.description || "")
      .join(" ")
      .toLowerCase();

    if (
      descriptions.includes("happy") ||
      descriptions.includes("satisfied") ||
      descriptions.includes("excellent")
    ) {
      return "positive";
    }

    if (
      descriptions.includes("angry") ||
      descriptions.includes("dissatisfied") ||
      descriptions.includes("complaint")
    ) {
      return "negative";
    }
  ...
  private extractTopicsFromActivities(activities: Activity[]): string[] {
    ...
    if (
      ...
      descriptions.includes("maintenance")
    ) {
      topics.push("Service");
    }
    ...
    return [
      {
        ...
        expected_response: {
          summary:
            "Customer has been actively engaged with 5 interactions this month",
          key_points: [
...
export function validateActivityQuery(query: ActivityQuery): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (query.type === "summary" || query.type === "weekly_digest") {
    if (!query.customerId) {
      errors.push(
        "Customer ID is required for summary and weekly digest queries",
      );
    }
  }
  ...
  if (query.type === "range_summary") {
    if (!query.customerId || !query.startDate || !query.endDate) {
      errors.push(
        "Customer ID, start date, and end date are required for range summary",
      );
    }
  }
