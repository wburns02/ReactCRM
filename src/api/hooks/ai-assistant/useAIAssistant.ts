...
export function useAIAssistant(conversationId?: string): UseAIAssistantReturn {
  ...
  const executeActionMutation = useMutation({
    mutationFn: async (action: AIAction): Promise<ActionResult> => {
      ...
      try {
        ...
        const error =
          err instanceof Error ? err : new Error("Action execution failed");
        setError(error);
        throw error;
      }
    },
  ...
  const updateConversationWithMessage = useCallback(
    async (userMessage: string, response: AIResponse): Promise<void> => {
      ...
      const assistantMsg: AIMessage = {
        ...
        role: "assistant",
        content:
          typeof response.primaryResult === "string"
  ...
  const streamMessage = useCallback(
    async function* (message: string): AsyncGenerator<Partial<AIResponse>> {
      if (!contextQuery.data) {
        throw new Error("Context not available");
      }
      ...
      try {
        for await (const chunk of orchestrator.streamQuery(
          message,
          contextQuery.data,
        )) {
          yield chunk;
  ...
  const isHealthy = healthQuery.data
    ? Object.values(healthQuery.data).some(
...
export function useAIActions() {
  const { executeAction, rollbackAction, getActionHistory, isLoading, error } =
    useAIAssistant();
...
export const aiAssistantKeys = {
  ...
  context: (conversationId?: string) =>
    [...aiAssistantKeys.all, "context", conversationId] as const,
  messages: (conversationId?: string) =>
    [...aiAssistantKeys.all, "messages", conversationId] as const,
  conversation: (conversationId?: string) =>
    [...aiAssistantKeys.all, "conversation", conversationId] as const,
