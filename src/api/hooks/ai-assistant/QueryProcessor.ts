...
export class QueryProcessor {
          ...
          if (match[0] && match.index !== undefined) {
            entities.push({
              ...
              metadata: {
                ...
                context: this.extractEntityContext(
                  query,
                  match.index,
                  match[0].length,
                ),
  ...
  async classifyIntent(
    ...
    const operation = this.identifyOperation(
      normalizedQuery,
      intentType,
      domain,
    );
  ...
  enhanceWithContext(intent: AIIntent, context: AIContext): AIIntent {
    ...
    if (context.session.conversationHistory.length > 0) {
      const recentContext = this.extractRecentContext(
  ...
  private classifyIntentType(query: string): AIIntent["type"] {
    // Question patterns
    if (
      /^(?:what|who|when|where|why|how|which|can|could|would|should)\b/.test(
        query,
      )
    ) {
      return "query";
    ...
    // Action patterns
    if (
      /^(?:create|add|schedule|assign|send|generate|update|delete|cancel)\b/.test(
        query,
      )
    ) {
      return "action";
  ...
  private determinePriority(
    ...
    if (
      customerEntity &&
      context.domain.customers?.some((c) => c.tier === "vip")
    ) {
      return "high";
    }
    ...
    for (const entity of sorted) {
      const overlapping = deduplicated.find(
        (existing) =>
          ...
          (existing.value.includes(entity.value) ||
            entity.value.includes(existing.value)),
      );
