...
export function useGenerateReviewResponse() {
  return useMutation({
    mutationFn: async (params: {
      ...
      try {
        const response = await apiClient.post("/ai/reviews/generate-response", params);
        return response.data;
      } catch {
...
export function useBulkGenerateResponses() {
  return useMutation({
    mutationFn: async (params: {
      review_ids: string[];
    }): Promise<{ generated: number; responses: Record<string, GeneratedResponse> }> => {
      try {
        const response = await apiClient.post("/ai/reviews/bulk-generate", params);
        return response.data;
      } catch {
        const responses: Record<string, GeneratedResponse> = {};
        params.review_ids.forEach(id => {
          responses[id] = generateDemoResponse({ review_text: "Thank you for your feedback!" });
        });
...
function generateDemoAnalysis(text: string, rating?: number): ReviewAnalysis {
  ...
  const isPositive = rating ? rating >= 4 : (
    ...
    lowerText.includes("amazing") || lowerText.includes("thank")
  );
  const isNegative = rating ? rating <= 2 : (
    lowerText.includes("terrible") || lowerText.includes("awful") ||
    lowerText.includes("worst") || lowerText.includes("never")
  );
  ...
  return {
    ...
    emotions_detected: isPositive ? ["satisfaction", "gratitude"] :
                       isNegative ? ["frustration", "disappointment"] :
                       ["neutral"],
    requires_response: true,
    response_priority: isNegative ? "high" : isPositive ? "low" : "medium",
    suggested_tone: isNegative ? "apologetic" : isPositive ? "grateful" : "professional",
  };
}
...
function extractTopics(text: string): string[] {
  ...
  if (lowerText.includes("service") || lowerText.includes("work")) topics.push("service quality");
  if (lowerText.includes("price") || lowerText.includes("cost") || lowerText.includes("expensive")) topics.push("pricing");
  if (lowerText.includes("time") || lowerText.includes("quick") || lowerText.includes("late")) topics.push("timeliness");
  if (lowerText.includes("professional") || lowerText.includes("technician") || lowerText.includes("staff")) topics.push("staff");
  if (lowerText.includes("clean") || lowerText.includes("mess")) topics.push("cleanliness");
...
function generateDemoResponse(params: {
  ...
  if (isPositive) {
    return {
      ...
      tone: "grateful",
      key_points_addressed: ["Expressed gratitude", "Acknowledged positive experience", "Invited future business"],
    ...
    return {
      ...
      tone: "apologetic",
      key_points_addressed: ["Acknowledged feedback", "Apologized", "Offered resolution", "Requested follow-up"],
  ...
  return [
    {
      ...
      template: "Thank you so much for your amazing {rating}-star review, {customer_name}! We're thrilled that {technician_name} and our team could provide excellent service. Your recommendation means the world to us!",
    ...
    },
    {
      ...
      sentiment_target: "positive",
      template: "We truly appreciate your kind feedback, {customer_name}! It's customers like you who make our work rewarding. Thank you for trusting us with your {service_type} needs. We look forward to serving you again!",
    ...
    },
    {
      ...
      template: "We sincerely apologize for falling short of your expectations, {customer_name}. Your feedback about {issue} is taken very seriously. We'd like to make this right - please contact our customer service team at {phone} so we can address your concerns directly.",
    ...
    },
    {
      ...
      template: "Thank you for your review, {customer_name}. We value your feedback and always strive to improve. If there's anything specific we can do better, please don't hesitate to reach out. We appreciate your business!",
