...
export const aiInsightsKeys = {
  ...
  campaignAnalysis: (campaignId: number) => [...aiInsightsKeys.all, 'campaign', campaignId] as const,
  customerInsight: (customerId: number) => [...aiInsightsKeys.all, 'customer', customerId] as const,
...
export function useCampaignAIAnalysis(campaignId: number | null) {
  return useQuery({
    ...
    queryFn: async (): Promise<CampaignAIAnalysis> => {
      const { data } = await apiClient.get(`/cs/ai/campaigns/${campaignId}/analysis`);
      return data;
    },
    enabled: !!campaignId,
    staleTime: 5 * 60 * 1000,
...
export function useCustomerInsight(customerId: number | null) {
  return useQuery({
    ...
    queryFn: async (): Promise<CustomerInsight> => {
      const { data } = await apiClient.get(`/cs/ai/customers/${customerId}/insight`);
      return data;
    },
    enabled: !!customerId,
    staleTime: 5 * 60 * 1000,
...
export function useSubjectSuggestions() {
  return useMutation({
    mutationFn: async (data: { subject: string; campaign_goal: string }): Promise<SubjectSuggestions> => {
...
export function useDismissRecommendation() {
  ...
  return useMutation({
    mutationFn: async (recommendationId: string): Promise<{ success: boolean }> => {
      const response = await apiClient.post(`/cs/ai/recommendations/${recommendationId}/dismiss`);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: aiInsightsKeys.recommendations() });
    },
  });
}
...
export function useApplyRecommendation() {
  ...
  return useMutation({
    mutationFn: async (recommendationId: string): Promise<{ success: boolean; result?: string }> => {
      const response = await apiClient.post(`/cs/ai/recommendations/${recommendationId}/apply`);
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: aiInsightsKeys.recommendations() });
      queryClient.invalidateQueries({ queryKey: aiInsightsKeys.portfolioInsights() });
    },
  });
}
