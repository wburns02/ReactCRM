...
export const fintechKeys = {
  ...
  cashFlow: {
    ...
    arAging: () => [...fintechKeys.cashFlow.all, "ar-aging"] as const,
...
export function useFinancingApplications(customerId?: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<FinancingApplication[]> => {
      ...
      const { data } = await apiClient.get("/financing/applications", {
        params,
      });
      return z
        .array(financingApplicationSchema)
        .parse(data.applications || data);
    },
  });
}
...
export function useRequestFinancing() {
  ...
  return useMutation({
    mutationFn: async (
      request: FinancingPrequalRequest,
...
export function useTechnicianPayouts(technicianId: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<TechnicianPayout[]> => {
      const { data } = await apiClient.get(
        `/payouts/technicians/${technicianId}`,
      );
...
export function useRevenueIntelligence(startDate?: string, endDate?: string) {
  return useQuery({
    ...
    queryFn: async (): Promise<RevenueIntelligence> => {
      ...
      const { data } = await apiClient.get("/analytics/revenue-intelligence", {
        params,
      });
...
export function useCollectionRecommendations() {
  return useQuery({
    ...
    queryFn: async (): Promise<{
      ...
      const { data } = await apiClient.get(
        "/analytics/collection-recommendations",
      );
      return data;
    },
