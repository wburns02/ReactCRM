...
export const serviceIntervalKeys = {
  ...
  list: (filters?: Record<string, unknown>) => [...serviceIntervalKeys.lists(), filters] as const,
...
export function useUpdateServiceInterval() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const { data } = await apiClient.put(`/service-intervals/${id}`, interval);
      return data;
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: serviceIntervalKeys.lists() });
      queryClient.invalidateQueries({ queryKey: serviceIntervalKeys.detail(variables.id) });
    },
  });
}
...
export function useAssignServiceInterval() {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      const { data } = await apiClient.post('/service-intervals/assign', params);
      return data;
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: serviceIntervalKeys.schedules() });
...
export function useCreateWorkOrderFromSchedule() {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      const { data } = await apiClient.post('/service-intervals/create-work-order', params);
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: serviceIntervalKeys.schedules() });
...
export function useSendServiceReminder() {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      const { data } = await apiClient.post('/service-intervals/send-reminder', params);
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: serviceIntervalKeys.reminders() });
      queryClient.invalidateQueries({ queryKey: serviceIntervalKeys.schedules() });
    },
  });
}
...
export function useBulkAssignServiceInterval() {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      const { data } = await apiClient.post('/service-intervals/bulk-assign', params);
      return data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: serviceIntervalKeys.schedules() });
