...
export function PortalWorkOrdersPage() {
  ...
  const filteredOrders = workOrders.filter(wo => {
    ...
    if (filter === 'upcoming') return wo.status === 'scheduled' || wo.status === 'in_progress';
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-text-secondary">View your service history and upcoming appointments</p>
        </div>
      ...
      {filteredOrders.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredOrders.map(wo => (
            <Link key={wo.id} to={`/portal/work-orders/${wo.id}`} className="block">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-4 text-sm text-text-muted">
                        ...
                        {wo.total_amount && (
                          <span className="flex items-center gap-1">
                            💰 ${wo.total_amount.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                          </span>
                        )}
                      </div>
                    ...
                    <div className="flex flex-col gap-2 items-end">
                      <span className="text-primary text-sm">View Details &rarr;</span>
                      {wo.completed_date && (
