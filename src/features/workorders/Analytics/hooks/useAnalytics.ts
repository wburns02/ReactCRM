 ...
 */
function generateMockCompletionTrend(
  dateRange: DateRange,
): CompletionTrendPoint[] {
...
function generateMockSatisfactionData(): {
  ...
  const reviews: CustomerReview[] = [
    ...
    },
    {
      ...
      rating: 4,
      comment:
        "Good work overall. Arrived on time and fixed the issue quickly.",
  ...
  const busyPeriodAlerts: BusyPeriodAlert[] = [
    {
      ...
      severity: "high",
      reason:
        "Spring maintenance season typically sees 80% increase in pumping requests",
    },
    {
      id: "2",
  ...
  const equipmentAlerts: EquipmentAlert[] = [
    {
      ...
      suggestedAction:
        "Schedule preventive maintenance. Replace seals and check motor bearings.",
      priority: "high",
    },
    {
      id: "2",
      ...
      suggestedAction:
        "Inspect tank integrity during next scheduled maintenance window.",
      priority: "medium",
    },
    {
      id: "3",
...
export function useCustomerSatisfaction(dateRange: DateRange) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const totalResponses = data.distribution.reduce(
        (sum, d) => sum + d.count,
        0,
      );
      const totalRating = data.distribution.reduce(
      ...
      const promoters = data.distribution
        .filter((d) => d.rating >= 4)
        .reduce((sum, d) => sum + d.count, 0);
      const detractors = data.distribution
        .filter((d) => d.rating <= 2)
        .reduce((sum, d) => sum + d.count, 0);
