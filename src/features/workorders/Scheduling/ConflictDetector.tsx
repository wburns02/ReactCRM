...
export function ConflictDetector({
  ...
  if (conflicts.length === 0) {
    ...
    return (
      <div className={cn('flex items-center gap-2 p-3 rounded-lg bg-success/10 border border-success/20', className)}>
        <span className="text-success text-lg">✓</span>
        <span className="text-sm text-success font-medium">No scheduling conflicts detected</span>
      </div>
    );
  }
    ...
    <div className={cn('space-y-3', className)}>
      ...
      <div
        className={cn(
          ...
          hasErrors ? 'bg-danger/10 border border-danger/20' : 'bg-warning/10 border border-warning/20'
        )}
      >
        <div className="flex items-center gap-2">
          <span className={cn('text-lg', hasErrors ? 'text-danger' : 'text-warning')}>
            {hasErrors ? '⚠️' : '⚡'}
          </span>
          <span
            className={cn(
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <span className="text-lg flex-shrink-0">{getConflictIcon(conflict.type)}</span>
        ...
        {onClick && (
          <svg
            className={cn('w-4 h-4 flex-shrink-0', isError ? 'text-danger' : 'text-warning')}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
    ...
    <Card className={className}>
      <CardContent className="p-4">
        <div className="flex items-center justify-between">
          <div className="text-sm font-medium text-text-primary">Scheduling Health</div>
