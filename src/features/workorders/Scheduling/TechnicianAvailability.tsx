...
interface TechnicianAvailabilityProps {
  onCellClick?: (technicianId: string, technicianName: string, date: string) => void;
  daysToShow?: number;
  className?: string;
}
    ...
    return (
      <Card className={className}>
        <CardContent className="py-12 text-center">
          ...
          <p className="text-text-secondary mt-4">Loading availability data...</p>
        </CardContent>
      </Card>
    );
  }
      ...
      <CardContent className="p-0">
            ...
            <thead>
              <tr>
                <th className="sticky left-0 z-10 bg-bg-muted border-b border-r border-border p-3 text-left min-w-[150px]">
                  <span className="text-sm font-medium text-text-secondary">Technician</span>
                </th>
                {dateHeaders.map((header) => (
                  ...
                  >
                    <div className="text-xs text-text-secondary">{header.dayName}</div>
                    <div
                      className={cn(
                        'text-sm font-medium',
                        header.isToday
                ...
                <th className="border-b border-border p-2 text-center min-w-[80px] bg-bg-muted">
                  <span className="text-xs font-medium text-text-secondary">Avg</span>
                </th>
              </tr>
            </thead>
            ...
            <tbody>
              {capacityData.length === 0 ? (
                ...
                capacityData.map((techData) => (
                  <tr key={techData.technician.id} className="hover:bg-bg-hover/50">
                    {/* Technician name */}
                    ...
                    {techData.days.map((day) => {
                      const header = dateHeaders.find((h) => h.date === day.date);
                      return (
                        <td
                          key={day.date}
                          className={cn(
        ...
        <div className="border-t border-border p-4 bg-bg-muted">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-2">
              <span className="text-xs text-text-secondary font-medium">Capacity:</span>
