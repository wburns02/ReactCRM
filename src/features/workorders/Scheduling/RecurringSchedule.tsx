...
export function RecurringSchedule({
  ...
  const [frequency, setFrequency] = useState<Frequency>(value?.frequency || 'monthly');
  ...
  const [endType, setEndType] = useState<'never' | 'date' | 'occurrences'>(
    value?.endDate ? 'date' : value?.endAfterOccurrences ? 'occurrences' : 'never'
  );
  ...
  }, [isEnabled, frequency, interval, daysOfWeek, dayOfMonth, endType, endDate, endAfterOccurrences]);
  ...
  const previewDates = useMemo(() => {
    ...
    const maxDate = value?.endDate ? parseISO(value.endDate) : addYears(new Date(), 2);
    ...
    return dates;
  }, [currentPattern, startDate, frequency, daysOfWeek, interval, maxPreviewDates]);
    ...
    switch (frequency) {
      ...
      case 'weekly':
        if (daysOfWeek.length > 0) {
          ...
          desc = interval === 1 ? `Weekly on ${dayNames}` : `Every ${interval} weeks on ${dayNames}`;
        } else {
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="recurring-enabled" className="text-sm text-text-secondary">
              Enable recurring
            </label>
          </div>
        </div>
        ...
        {isEnabled ? (
          <>
                  ...
                  {endType === 'occurrences' && (
                    <>
                      <Input
                        ...
                        onChange={(e) =>
                          setEndAfterOccurrences(Math.max(1, parseInt(e.target.value) || 1))
                        }
                        className="w-20"
                      />
            ...
            <div className="p-3 bg-bg-muted rounded-lg">
              <div className="text-sm text-text-secondary">Schedule summary:</div>
          ...
          <div className="text-center py-8 text-text-muted">
            ...
            <div className="text-sm">Enable recurring to set up a schedule pattern</div>
          </div>
        )}
      </CardContent>
    </Card>
  ...
  const getLabel = () => {
    switch (pattern.frequency) {
      ...
      case 'biweekly':
        return 'Bi-weekly';
      case 'monthly':
        return pattern.interval === 1 ? 'Monthly' : `Every ${pattern.interval}mo`;
      case 'quarterly':
        return 'Quarterly';
      case 'yearly':
