...
const SMS_TEMPLATES = [
  ...
  },
  {
    ...
    content:
      "Hi {{customer_name}}, this is MAC Septic. Your appointment is scheduled for {{appointment_date}} at {{appointment_time}}. Reply STOP to opt out.",
  },
  {
    id: "enroute",
    name: "Tech En Route",
    content:
      "Hi {{customer_name}}, your technician is on the way and should arrive in approximately {{eta_minutes}} minutes. Thank you for choosing MAC Septic!",
  },
  {
    id: "complete",
    name: "Service Complete",
    content:
      "Hi {{customer_name}}, your service has been completed. Thank you for your business! Your invoice will be emailed shortly. Reply STOP to opt out.",
  },
  {
    id: "payment",
    name: "Payment Reminder",
    content:
      "Hi {{customer_name}}, this is a friendly reminder that your invoice of {{invoice_amount}} is due. Pay online at: {{invoice_link}}. Reply STOP to opt out.",
  },
];
  ...
  const sendSMS = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["communications", "conversation", customerId],
      });
    },
    onError: (error: Error) => {
      ...
      <CardContent>
        {!isConnected && !statusLoading ? (
          <div className="text-center py-6 text-text-secondary">
            <svg
              className="w-12 h-12 mx-auto mb-3 text-warning"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            <p className="text-sm">
              {twilioStatus?.message || "Please configure Twilio credentials"}
            </p>
          </div>
        ) : (
