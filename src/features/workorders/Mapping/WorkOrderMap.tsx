...
function createWorkOrderMarker(
...
function createTechnicianMarker(
  status: TechnicianLocation["status"],
  avatarUrl?: string,
): L.DivIcon {
  const statusColors = {
    active: "#22c55e",
...
function MapBoundsUpdater({
  ...
  useEffect(() => {
    ...
    if (selectedId) {
      ...
      if (
        selected?.service_latitude != null &&
        selected?.service_longitude != null
      ) {
...
export function WorkOrderMap({
  ...
  const renderWorkOrderMarkers = () => {
    return validWorkOrders.map((workOrder) => {
      ...
      const markerIcon = createWorkOrderMarker(
        workOrder.status,
    ...
    >
      ...
      <div className="absolute top-4 right-4 bg-white/95 backdrop-blur-sm rounded-lg shadow-lg px-3 py-2 z-[1000]">
        <p className="text-sm font-medium text-gray-700">
          {validWorkOrders.length} work order
          {validWorkOrders.length !== 1 ? "s" : ""}
        </p>
      </div>
    </div>
  );
