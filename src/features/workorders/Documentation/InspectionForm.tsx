...
import type { InspectionItem, InspectionForm as InspectionFormType, PhotoType } from '@/api/types/workOrder';
    ...
    <div className={cn('space-y-6', className)}>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-text-primary">{form.templateName}</h3>
            <p className="text-sm text-text-muted">
              {progress.completed}/{progress.total} items completed
              {progress.required > 0 && ` (${progress.completedRequired}/${progress.required} required)`}
            </p>
          </div>
              ...
              >
                ...
                <div className="w-20 h-1.5 bg-bg-muted rounded-full overflow-hidden">
                  <div
                    className={cn(
                      'h-full',
                      categoryProgress.percentage === 100 ? 'bg-success' : 'bg-primary'
                    )}
                ...
                <div className="divide-y divide-border">
                  {categoryItems.map(item => (
                    <InspectionFormItem
                      ...
                      onCapturePhoto={onCapturePhoto ? () => onCapturePhoto(item.id, 'other') : undefined}
                      readOnly={readOnly}
                    />
                  ))}
                </div>
