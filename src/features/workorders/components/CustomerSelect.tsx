...
export function CustomerSelect({ value, onChange, onAddNew, disabled }: CustomerSelectProps) {
    ...
    <div className="space-y-3" ref={containerRef}>
      <div className="flex gap-2">
        ...
        <div className="flex-1 relative">
          <Input
            value={selectedCustomer ? `${selectedCustomer.first_name} ${selectedCustomer.last_name}` : search}
            onChange={(e) => {
          ...
          {isOpen && (
            <div className="absolute z-50 w-full mt-1 bg-bg-card border border-border rounded-lg shadow-lg max-h-64 overflow-auto">
              ...
              {filteredCustomers.length === 0 && (
                <div className="px-4 py-2 text-text-muted">No customers found</div>
              )}
            </div>
          )}
        </div>
        ...
        {/* Quick-add button */}
        {onAddNew && (
          <Button type="button" variant="secondary" onClick={onAddNew} disabled={disabled}>
            +
          </Button>
        )}
      </div>
