 ...
 */
export function PaymentInsightsPanel({
  customerId,
}: PaymentInsightsPanelProps) {
  const [showPanel, setShowPanel] = useState(false);
  const [activeTab, setActiveTab] = useState<"pattern" | "collection">(
    customerId ? "pattern" : "collection",
  );

  const { data: pattern, isLoading: patternLoading } =
    usePaymentPattern(customerId);
  const {
    data: collection,
  ...
  const isLoading =
    activeTab === "pattern" ? patternLoading : collectionLoading;
    ...
    <div className="bg-gradient-to-r from-purple-500/10 to-blue-500/10 border border-purple-500/30 rounded-lg p-4">
      ...
      {activeTab === "pattern" && pattern && !isLoading && (
        <div className="space-y-4">
          {/* Risk Assessment */}
          <div
            className={`p-4 rounded-lg border ${getRiskColor(pattern.risk_assessment.risk_level)}`}
          >
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm">Payment Risk</span>
              <span className="text-xl font-bold uppercase">
                {pattern.risk_assessment.risk_level}
              </span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">
                {pattern.risk_assessment.risk_score}/100
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-bg-card border border-border rounded-lg p-3">
              ...
              <span
                className={`text-lg font-bold capitalize ${getBehaviorColor(pattern.payment_behavior.typical_payment_timing)}`}
              >
                {pattern.payment_behavior.typical_payment_timing.replace(
                  "_",
                  " ",
                )}
              </span>
          ...
          <div className="bg-bg-card border border-border rounded-lg p-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="flex justify-between">
                ...
                <span
                  className={
                    pattern.historical_stats.paid_on_time_rate >= 80
                      ? "text-green-400"
                      : "text-yellow-400"
                  }
                >
              ...
              <div className="flex justify-between">
                ...
                <span className="text-text-primary">
                  {pattern.historical_stats.total_invoices}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-text-muted">Lifetime value:</span>
                <span className="text-success">
                  ${pattern.historical_stats.lifetime_value.toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span
                  className={
                    pattern.historical_stats.outstanding_balance > 0
                      ? "text-warning"
                      : "text-success"
                  }
                >
        ...
        <div className="space-y-4">
          ...
          <div className="bg-bg-card border border-border rounded-lg p-4">
            ...
            <p className="text-xs text-success">
              Predicted collections (30 days): $
              {collection.predicted_collections_30_days.toLocaleString()}
            </p>
          </div>
            ...
            <div className="grid grid-cols-5 gap-1 text-xs">
              {collection.aging_breakdown.map((bucket) => (
                <div key={bucket.bucket} className="text-center">
                  <span
                    className={`block ${getAgingColor(bucket.bucket).replace("bg-", "text-")}`}
                  >
                    {bucket.bucket}
                  </span>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-warning">
                        ${account.outstanding_amount.toLocaleString()}
                      </span>
                      <span className="text-xs text-text-muted">
          ...
          {collection.collection_priority_queue.length > 0 && (
            <div>
              ...
              <div className="space-y-2">
                {collection.collection_priority_queue
                  .slice(0, 3)
                  .map((item, i) => (
                    <div
                    ...
                    >
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-sm text-warning ml-auto">
                          ${item.amount.toLocaleString()}
                        </span>
                      </div>
                      <p className="text-xs text-text-muted mb-1">
                        Best time: {item.best_contact_time}
                      </p>
                  ...
                  >
                    <div>
                      ...
                      <span className="text-xs text-text-muted ml-2">
                        ({action.target_accounts} accounts)
                      </span>
                    </div>
                    <div className="text-right">
                      <span className="text-sm font-medium text-success">
                        ${action.expected_recovery.toLocaleString()}
                      </span>
