...
export function SegmentDetail({
  ...
  const { data: analyticsData, isLoading: analyticsLoading } =
    useSegmentAnalytics(segment.id);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {/* Insights Tab */}
        {activeTab === "insights" && <InsightsTab segment={segment} />}
      </div>
    </div>
  );
}
              ...
              {members.map((member) => (
                ...
                >
                    ...
                    {member.health_score !== null && (
                      <span
                        className={cn(
                          ...
                          member.health_score >= 70
                            ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"
                            : member.health_score >= 40
                  ...
                  <td className="py-3 px-4 text-sm text-gray-500 dark:text-gray-400">
                    {member.joined_segment_at
                      ? new Date(member.joined_segment_at).toLocaleDateString()
                      : "-"}
                  </td>
                  <td className="py-3 px-4">
    ...
    <div className="grid gap-6 md:grid-cols-2">
      ...
      <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
        ...
        <div className="h-40 flex items-end gap-1">
          {analytics.size_over_time.map((point, index) => {
            const max = Math.max(
              ...analytics.size_over_time.map((p) => p.count),
            );
        ...
        <div className="flex justify-between mt-2 text-xs text-gray-500 dark:text-gray-400">
          {analytics.size_over_time.length > 0 && (
            <>
              <span>
                {new Date(
                  analytics.size_over_time[0].date,
                ).toLocaleDateString()}
              </span>
              <span>
                {new Date(
                  analytics.size_over_time[analytics.size_over_time.length - 1]
                    .date,
                ).toLocaleDateString()}
              </span>
            </>
      ...
      <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4">
        ...
        <div className="space-y-3">
          {Object.entries(analytics.health_distribution).map(
            ([status, count]) => {
              const total = Object.values(analytics.health_distribution).reduce(
                (a, b) => a + b,
                0,
              );
...
function InsightsTab({ segment }: InsightsTabProps) {
  ...
  const insights = [
    ...
    },
    {
      ...
      description:
        "Average engagement score for this segment has dropped 8% over the last 30 days.",
      action: "Investigate",
    },
    {
      id: "3",
      ...
      description:
        'Based on segment characteristics, consider running the "Quarterly Business Review" playbook.',
