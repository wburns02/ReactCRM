...
export const mockSegmentAIInsights: Record<number, SegmentAIInsight> = {
  1: {
    ...
    confidence: 0.92,
    insights: {
      key_finding: 'High Value Accounts show 15% higher engagement than average. Three accounts are approaching renewal with strong expansion signals.',
      opportunity: 'Cross-sell aerobic system upgrades to 5 accounts that would benefit from advanced monitoring.',
      recommended_action: 'Schedule QBRs with top 5 accounts to discuss expansion opportunities before Q3.',
  ...
  2: {
    ...
    confidence: 0.88,
    insights: {
      key_finding: 'At-Risk segment has grown by 4 customers in the last 30 days. Common pattern: delayed service responses.',
      risk_assessment: '3 customers show signs of evaluating competitors based on recent touchpoint analysis.',
      recommended_action: 'Immediately reach out to the 3 highest-risk accounts. Deploy service recovery playbook.',
  ...
  102: {
    ...
    confidence: 0.95,
    insights: {
      key_finding: 'VIP At-Risk customers represent $62,500 in ARR at immediate risk. Average time in segment: 18 days.',
      risk_assessment: 'Without intervention, models predict 60% likelihood of churn within 90 days for this segment.',
      recommended_action: 'Executive sponsor outreach within 48 hours. Consider service credits or enhanced support.',
      action_priority: 'critical',
    },
    trends: {
      size_trend: 'growing',
  ...
  104: {
    ...
    confidence: 0.91,
    insights: {
      key_finding: 'Contract renewals due segment shows 85% historical renewal rate. 3 accounts have not responded to initial outreach.',
      opportunity: 'Multi-year contract conversion opportunity for 6 accounts with strong health scores.',
      recommended_action: 'Prioritize personal outreach to non-responsive accounts. Prepare renewal incentive packages.',
      action_priority: 'high',
    },
    trends: {
      size_trend: 'stable',
  ...
  105: {
    ...
    confidence: 0.87,
    insights: {
      key_finding: 'Recent complainers primarily cite response time (57%) and service quality (43%) as concerns.',
      risk_assessment: 'Complaint resolution within 72 hours reduces churn probability by 65%.',
      recommended_action: 'Deploy service recovery playbook. Assign dedicated CSM for high-touch follow-up.',
      action_priority: 'critical',
    },
    trends: {
      size_trend: 'stable',
 ...
 */
export function getMockSegmentInsights(segmentId: number): SegmentAIInsight | undefined {
 ...
 */
export function filterMockSegmentsByType(type: 'static' | 'dynamic' | 'ai_generated' | 'all'): Segment[] {
...
export function getHighPriorityMockSegments(): Segment[] {
  return mockSegments.filter((s) => s.priority >= 90).sort((a, b) => b.priority - a.priority);
}
