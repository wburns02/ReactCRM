...
const ALL_OPERATORS: OperatorDefinition[] = [
  ...
  { id: 'eq', label: 'equals', description: 'Exactly matches', types: ['string', 'number', 'boolean', 'date', 'select'] },
  { id: 'neq', label: 'not equals', description: 'Does not match', types: ['string', 'number', 'boolean', 'date', 'select'] },

  // Comparison operators (number, date)
  { id: 'gt', label: 'greater than', description: '>', types: ['number', 'date'] },
  { id: 'lt', label: 'less than', description: '<', types: ['number', 'date'] },
  { id: 'gte', label: 'greater or equal', description: '>=', types: ['number', 'date'] },
  { id: 'lte', label: 'less or equal', description: '<=', types: ['number', 'date'] },
  { id: 'between', label: 'between', description: 'In range', types: ['number', 'date'] },

  // String operators
  { id: 'contains', label: 'contains', description: 'Includes text', types: ['string'] },
  { id: 'not_contains', label: 'does not contain', description: 'Excludes text', types: ['string'] },
  { id: 'starts_with', label: 'starts with', description: 'Begins with', types: ['string'] },
  { id: 'ends_with', label: 'ends with', description: 'Ends with', types: ['string'] },

  // Multi-value operators
  { id: 'in', label: 'is one of', description: 'Matches any', types: ['string', 'number', 'select'] },
  { id: 'not_in', label: 'is not one of', description: 'Excludes all', types: ['string', 'number', 'select'] },

  // Null operators (all types)
  { id: 'is_null', label: 'is empty', description: 'No value', types: ['string', 'number', 'boolean', 'date', 'select'] },
  { id: 'is_not_null', label: 'has value', description: 'Any value', types: ['string', 'number', 'boolean', 'date', 'select'] },
];
...
export function OperatorSelect({ value, fieldType, onChange, className }: OperatorSelectProps) {
  const [isOpen, setIsOpen] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);

  // Filter operators based on field type
  const availableOperators = ALL_OPERATORS.filter(op => op.types.includes(fieldType));
        ...
        <div className="absolute z-50 w-48 mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl overflow-hidden">
          <div className="max-h-60 overflow-y-auto p-1">
            {availableOperators.map((op) => (
              ...
              >
                <div className="font-medium text-gray-900 dark:text-white">{op.label}</div>
                {op.description && (
 ...
 */
export function getOperatorById(id: RuleOperator): OperatorDefinition | undefined {
