...
export function RuleGroup({
  ...
  const isRuleSet = (
    item: SegmentRule | SegmentRuleSet,
  ): item is SegmentRuleSet => {
          ...
          <li key={index} className="flex items-center gap-2">
            ...
            {isRuleSet(item) ? (
              <span className="italic">
                Nested group ({countRules(item)} rules)
              </span>
            ) : (
 ...
 */
export function validateRuleSet(ruleSet: SegmentRuleSet): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
