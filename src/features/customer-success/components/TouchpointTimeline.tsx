...
function TouchpointItem({
  ...
  const typeConfig =
    TYPE_CONFIG[touchpoint.touchpoint_type] || TYPE_CONFIG.custom;
  const sentimentConfig = touchpoint.sentiment
    ? SENTIMENT_CONFIG[touchpoint.sentiment]
    : null;
  ...
  return (
    <div className="relative flex gap-4">
      ...
      {/* Content */}
      <div
        className={cn("flex-1 pb-6 min-w-0", onClick && "cursor-pointer")}
        onClick={onClick}
      >
        ...
        <div className="flex items-center gap-3 mt-2 flex-wrap">
          ...
          {touchpoint.sentiment_score !== null &&
            touchpoint.sentiment_score !== undefined && (
              <span
                className={cn(
                  "text-xs",
                  touchpoint.sentiment_score > 0
                    ? "text-success"
                    : touchpoint.sentiment_score < 0
                      ? "text-danger"
                      : "text-text-muted",
              ...
              >
                Score: {touchpoint.sentiment_score > 0 ? "+" : ""}
                {touchpoint.sentiment_score.toFixed(1)}
              </span>
            )}
        </div>
