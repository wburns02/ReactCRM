...
function SurveyStatusBadge({ status }: { status: SurveyStatus }) {
  ...
  return (
    <span
      className={cn("px-2 py-1 text-xs font-medium rounded-full", bg, text)}
    >
      {label}
    </span>
  );
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {(["nps", "csat", "ces", "custom"] as SurveyType[]).map(
                (type) => (
                  <button
                    key={type}
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-muted">
            Create and manage NPS, CSAT, and CES surveys
          </p>
        </div>
        <button
      ...
      {selectedSurvey && (
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-bg-card rounded-xl border border-border p-6">
            ...
            <ResponsesList
              responses={sampleResponses.filter(
                (r) => r.survey_id === selectedSurvey.id,
              )}
            />
          </div>
          ...
          {selectedSurvey.type === "nps" && (
            <NPSScoreBreakdown promoters={65} passives={20} detractors={15} />
          )}
        </div>
      )}
