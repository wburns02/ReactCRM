        ...
        <div className="mt-3 pt-3 border-t border-border">
          ...
          <div className="mt-1.5 h-2 bg-bg-hover rounded-full overflow-hidden">
            <div
              className={cn(
                "h-full rounded-full",
                colorClasses[color].replace("/10", ""),
              )}
              style={{
                width: `${Math.min(100, (Number(String(value).replace(/[^0-9.-]/g, "")) / target.value) * 100)}%`,
              }}
            />
          </div>
        </div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <KPICard
        ...
        target={{
          value: kpiData.nps.target,
          label: String(kpiData.nps.target),
        }}
        color="primary"
        isLoading={isLoading}
        icon={
      ...
      <KPICard
        ...
        trend={getTrendIcon(
          ...
          kpiData.churnRate.previous,
          false,
        )}
        target={{
          value: kpiData.churnRate.target,
          label: `${kpiData.churnRate.target}%`,
        }}
        color={
          kpiData.churnRate.current > 3
            ? "danger"
            : kpiData.churnRate.current > 2
              ? "warning"
              : "success"
        }
        isLoading={isLoading}
      ...
      <KPICard
        ...
        trend={getTrendIcon(
          kpiData.adoptionRate.current,
          kpiData.adoptionRate.previous,
        )}
        target={{
          value: kpiData.adoptionRate.target,
          label: `${kpiData.adoptionRate.target}%`,
        }}
        color="success"
        isLoading={isLoading}
        icon={
      ...
      <KPICard
        ...
        trend={getTrendIcon(
          kpiData.timeToValue.previousDays,
          kpiData.timeToValue.averageDays,
        )}
        target={{
          value: kpiData.timeToValue.targetDays,
          label: `${kpiData.timeToValue.targetDays} days`,
        }}
        color="info"
        isLoading={isLoading}
        icon={
      ...
      <KPICard
        ...
        trend={getTrendIcon(
          kpiData.engagementScore.current,
          kpiData.engagementScore.previous,
        )}
        target={{
          value: kpiData.engagementScore.target,
          label: `${kpiData.engagementScore.target}/10`,
        }}
        color="primary"
        isLoading={isLoading}
        icon={
      ...
      <KPICard
        ...
        trend={getTrendIcon(
          kpiData.expansionRevenue.current,
          kpiData.expansionRevenue.previous,
        )}
        target={{
          value: kpiData.expansionRevenue.target,
          label: formatCurrency(kpiData.expansionRevenue.target),
        }}
        color="success"
        isLoading={isLoading}
        icon={
