...
const OPERATORS: { value: LogicOperator; label: string; requiresValue: boolean; applicableTypes: string[] }[] = [
  { value: 'equals', label: 'equals', requiresValue: true, applicableTypes: ['rating', 'scale', 'single_choice', 'text'] },
  { value: 'not_equals', label: 'does not equal', requiresValue: true, applicableTypes: ['rating', 'scale', 'single_choice', 'text'] },
  { value: 'greater_than', label: 'is greater than', requiresValue: true, applicableTypes: ['rating', 'scale'] },
  { value: 'less_than', label: 'is less than', requiresValue: true, applicableTypes: ['rating', 'scale'] },
  { value: 'greater_or_equal', label: 'is greater than or equal to', requiresValue: true, applicableTypes: ['rating', 'scale'] },
  { value: 'less_or_equal', label: 'is less than or equal to', requiresValue: true, applicableTypes: ['rating', 'scale'] },
  { value: 'contains', label: 'contains', requiresValue: true, applicableTypes: ['text', 'multiple_choice'] },
  { value: 'not_contains', label: 'does not contain', requiresValue: true, applicableTypes: ['text', 'multiple_choice'] },
  { value: 'is_answered', label: 'is answered', requiresValue: false, applicableTypes: ['rating', 'scale', 'text', 'single_choice', 'multiple_choice'] },
  { value: 'is_not_answered', label: 'is not answered', requiresValue: false, applicableTypes: ['rating', 'scale', 'text', 'single_choice', 'multiple_choice'] },
];

const ACTIONS: { value: LogicAction; label: string; icon: React.ReactNode; requiresTarget: boolean }[] = [
  {
    value: 'show',
...
function RuleEditor({
  ...
  const sourceQuestion = questions.find((q) => q.id === rule.source_question_id);

  // Get applicable operators for the source question type
  const applicableOperators = useMemo(() => {
    if (!sourceQuestion) return OPERATORS;
    return OPERATORS.filter((op) => op.applicableTypes.includes(sourceQuestion.type));
  }, [sourceQuestion]);
  ...
  const renderValueInput = () => {
    ...
    if (sourceQuestion.type === 'scale' || sourceQuestion.type === 'rating') {
      const min = sourceQuestion.scale_min ?? (sourceQuestion.type === 'rating' ? 1 : 0);
      const max = sourceQuestion.scale_max ?? (sourceQuestion.type === 'rating' ? 5 : 10);
      const options = Array.from({ length: max - min + 1 }, (_, i) => min + i);

      return (
        <select
          value={rule.value as number}
          onChange={(e) => onUpdate({ ...rule, value: parseInt(e.target.value) })}
      ...
      {showLogicConnector && !isFirst && (
        <div className="flex items-center gap-2 mb-2">
          ...
          <select
            ...
            onChange={(e) => onUpdate({ ...rule, logic_group: e.target.value as 'and' | 'or' })}
      ...
      <div className="bg-bg-hover rounded-lg p-4">
        <div className="flex items-start gap-2 flex-wrap">
          ...
          <select
            ...
            onChange={(e) => onUpdate({ ...rule, source_question_id: e.target.value })}
          ...
          <select
            value={rule.operator}
            onChange={(e) => onUpdate({ ...rule, operator: e.target.value as LogicOperator })}
          ...
          {selectedAction?.requiresTarget && (
            <select
              ...
              onChange={(e) => onUpdate({ ...rule, target_question_id: e.target.value })}
              className="px-3 py-2 border border-border rounded-lg bg-bg-primary text-text-primary focus:outline-none focus:ring-2 focus:ring-primary max-w-[200px]"
            >
              <option value="">Select question...</option>
              {questions.map((q) => (
...
function LogicFlowDiagram({
  ...
  return (
    <div className="mt-4 p-4 bg-bg-hover rounded-lg">
      <h4 className="text-sm font-medium text-text-secondary mb-3">Logic Flow</h4>
            ...
            <div key={rule.id} className="flex items-center gap-2 text-sm">
              {index > 0 && (
                <span className={cn(
                  ...
                  rule.logic_group === 'or' ? 'bg-warning/10 text-warning' : 'bg-info/10 text-info'
                )}>
              ...
              <span className="text-text-primary font-medium truncate max-w-[150px]" title={sourceQ?.text}>
                {sourceQ?.text.substring(0, 20)}{(sourceQ?.text.length || 0) > 20 ? '...' : ''}
              </span>
...
function ValidationMessages({ rules, questions }: { rules: ConditionalLogicRule[]; questions: SurveyConditionalLogicProps['questions'] }) {
  const validationIssues = useMemo(() => {
    ...
    rules.forEach((rule, index) => {
      ...
      if (operator?.requiresValue && !rule.value && rule.value !== 0) {
        issues.push(`Rule ${index + 1}: Value is required for "${operator.label}" operator`);
      }

      if (rule.action === 'skip_to' && !rule.target_question_id) {
        issues.push(`Rule ${index + 1}: Target question required for "Skip to question" action`);
      }

      // Check for circular references
      if (rule.source_question_id === rule.target_question_id) {
        issues.push(`Rule ${index + 1}: Source and target questions cannot be the same`);
      }
    });

    // Check for conflicting rules
    const showHideConflicts = rules.filter((r) => r.action === 'show' || r.action === 'hide');
    const hasConflict = showHideConflicts.some((r1, i1) =>
      showHideConflicts.some((r2, i2) =>
        i1 !== i2 &&
    ...
    if (hasConflict) {
      issues.push('Conflicting rules detected: Same condition triggers both show and hide');
    }
...
export function SurveyConditionalLogic({
  ...
  const handleUpdateRule = (index: number, updatedRule: ConditionalLogicRule) => {
    ...
    <div className="space-y-4 pt-4 border-t border-border">
      ...
      {rules.length === 0 ? (
        <div className="text-center py-6 bg-bg-hover rounded-lg">
          ...
          <p className="text-xs text-text-muted mt-1">This question will always be displayed</p>
        </div>
      ) : (
      ...
      {/* Visual Flow Diagram */}
      <LogicFlowDiagram rules={rules} questions={questions} currentQuestionId={questionId} />
