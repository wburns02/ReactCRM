...
function SentimentIndicator({
  ...
  sentiment: AIGuidance["sentiment"];
}) {
  ...
  return (
    <div
      className={cn(
        ...
        colorClasses[slaStatus.color as keyof typeof colorClasses] ||
          colorClasses.gray,
      )}
    >
      <p className="text-sm font-medium">{slaStatus.message}</p>
      <p className="text-xs opacity-70 capitalize">
        {slaStatus.status.replace("_", " ")}
      </p>
    </div>
  );
}
  ...
  const {
    ...
    queryFn: async () => {
      const response = await apiClient.get(
        `/cs/escalations/${escalationId}/ai-guidance`,
      );
      ...
      <div className="flex border-b border-border">
        {[
          ...
          { id: "script", label: "📝 Full Script", icon: null },
          {
            id: "playbook",
            label: "✅ Playbook",
            icon: guidance.playbook
              ? ` (${guidance.playbook.steps.length} steps)`
              : "",
          },
        ].map((tab) => (
          <button
            ...
            <div className="bg-bg-card border border-border rounded-xl p-4">
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                ...
                <div>
                  <p className="text-2xl font-bold text-text-primary">
                    $
                    {guidance.customer_context.lifetime_value?.toLocaleString() ||
                      "0"}
                  </p>
