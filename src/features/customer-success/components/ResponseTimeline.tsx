...
const SENTIMENT_CONFIG: Record<ResponseSentiment, { label: string; color: string; bgColor: string; borderColor: string }> = {
  positive: { label: 'Positive', color: 'text-success', bgColor: 'bg-success/10', borderColor: 'border-success' },
  neutral: { label: 'Neutral', color: 'text-warning', bgColor: 'bg-warning/10', borderColor: 'border-warning' },
  negative: { label: 'Negative', color: 'text-danger', bgColor: 'bg-danger/10', borderColor: 'border-danger' },
};

// Score interpretation helper
function getScoreInfo(score: number, type: SurveyResponseType): { label: string; color: string } {
  if (type === 'nps') {
    if (score >= 9) return { label: 'Promoter', color: 'text-success bg-success/10' };
    if (score >= 7) return { label: 'Passive', color: 'text-warning bg-warning/10' };
    return { label: 'Detractor', color: 'text-danger bg-danger/10' };
  }
  if (type === 'csat') {
    if (score >= 4) return { label: 'Satisfied', color: 'text-success bg-success/10' };
    if (score >= 3) return { label: 'Neutral', color: 'text-warning bg-warning/10' };
    return { label: 'Unsatisfied', color: 'text-danger bg-danger/10' };
  }
  if (type === 'ces') {
    if (score >= 5) return { label: 'Easy', color: 'text-success bg-success/10' };
    if (score >= 4) return { label: 'Moderate', color: 'text-warning bg-warning/10' };
...
function formatRelativeDate(dateString: string): { date: string; time: string; relative: string } {
...
function generateSampleResponses(): TimelineResponse[] {
  ...
  const surveyTypes: SurveyResponseType[] = ['nps', 'csat', 'ces', 'nps', 'csat'];
  const sentiments: ResponseSentiment[] = ['positive', 'positive', 'positive', 'neutral', 'neutral', 'negative'];
  ...
  return Array.from({ length: 15 }, (_, i) => {
    ...
    let score: number;
    if (surveyType === 'nps') {
      score = sentiment === 'positive' ? 9 + Math.floor(Math.random() * 2) :
              sentiment === 'neutral' ? 7 + Math.floor(Math.random() * 2) :
              Math.floor(Math.random() * 7);
    } else if (surveyType === 'csat') {
      score = sentiment === 'positive' ? 4 + Math.floor(Math.random() * 2) :
              sentiment === 'neutral' ? 3 :
              1 + Math.floor(Math.random() * 2);
    } else {
      score = sentiment === 'positive' ? 5 + Math.floor(Math.random() * 3) :
              sentiment === 'neutral' ? 4 :
    ...
    return {
      ...
      surveyName: surveyType === 'nps' ? 'Q1 NPS Survey' :
                  surveyType === 'csat' ? 'Post-Support CSAT' :
                  surveyType === 'ces' ? 'Support CES' : 'Product Survey',
      surveyType,
      customerId: 100 + i,
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {response.customerCompany && (
                <span className="text-sm text-text-muted">at {response.customerCompany}</span>
              )}
              {!response.isRead && (
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-text-muted text-sm mt-1">
              {filter !== 'all' ? 'Try a different filter' : 'Responses will appear here as they come in'}
            </p>
          </div>
        ) : (
          <div>
      ...
      {showLoadMore && hasMore && !isLoading && (
        <div className="px-6 py-3 border-t border-border">
          ...
          >
            ...
            Load More ({filteredResponses.length - displayedResponses.length} remaining)
          </button>
        </div>
      )}
    </div>
