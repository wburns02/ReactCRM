...
// Types
export type DetractorActionStatus =
  | "needs_action"
  | "in_progress"
  | "resolved"
  | "escalated";
...
function generateSampleDetractors(): Detractor[] {
  ...
  const statuses: DetractorActionStatus[] = [
    "needs_action",
    "needs_action",
    "in_progress",
    "escalated",
  ...
  return names.map((n, i) => ({
    ...
    assignedTo:
      i % 3 === 0 ? "Alex Thompson" : i % 3 === 1 ? "Maria Garcia" : undefined,
...
function DetractorRow({
  ...
  const segmentConfig = detractor.segment
    ? SEGMENT_CONFIG[detractor.segment]
    : null;
...
export function DetractorQueue({
  ...
  const handleStatusUpdate = useCallback(
    (detractor: Detractor, newStatus: DetractorActionStatus) => {
      if (propDetractors) {
        onUpdateStatus?.(detractor, newStatus);
      } else {
        setDetractors((prev) =>
          prev.map((d) =>
            d.id === detractor.id ? { ...d, actionStatus: newStatus } : d,
          ),
        );
      }
    },
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-text-muted mt-0.5">
              {activeDetractors.length} detractors - {statusCounts.needs_action}{" "}
          ...
          <select
            value={sortBy}
            onChange={(e) =>
              setSortBy(e.target.value as "score" | "time" | "mrr")
            }
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-text-secondary font-medium">
              {filter === "all"
                ? "No detractors found"
                : `No ${filter.replace("_", " ")} detractors`}
            </p>
            <p className="text-text-muted text-sm mt-1">
              {filter === "all"
                ? "Great job! All customers are happy."
      ...
      {!isLoading && filteredDetractors.length > 0 && (
        <div className="px-6 py-3 border-t border-border bg-bg-hover/50">
          <div className="flex items-center justify-between text-xs text-text-muted">
            <span>
              Showing {filteredDetractors.length} of {activeDetractors.length}{" "}
              detractors
            </span>
