...
const TYPE_CONFIG: Record<SurveyType, { label: string; color: string; bgColor: string }> = {
  ...
  custom: { label: 'Custom', color: 'text-text-secondary', bgColor: 'bg-bg-hover' },
};

const STATUS_CONFIG: Record<SurveyStatus, { label: string; color: string; bgColor: string }> = {
...
function TrendSparkline({ data, className }: { data: number[]; className?: string }) {
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          {responseCount > survey.responses_count && (
            <span className="ml-1 text-xs text-success animate-pulse">+{responseCount - survey.responses_count}</span>
          )}
        </div>
      ...
      {survey.target_segment_name && (
        <div className="mb-4 pb-4 border-b border-border">
          <span className="text-xs text-text-muted">
            Target: <span className="text-text-secondary font-medium">{survey.target_segment_name}</span>
          </span>
        </div>
      )}
        ...
        <div className="absolute left-0 right-0 -bottom-2 transform translate-y-full z-10 px-4">
          <div className="bg-bg-primary border border-border rounded-lg shadow-lg p-3 text-xs">
            <div className="flex items-center justify-between mb-1">
              <span className="font-medium text-text-primary">{lastResponse.customerName}</span>
              <span className={cn(
                'px-1.5 py-0.5 rounded font-bold',
                lastResponse.score >= 9 ? 'bg-success/10 text-success' :
                lastResponse.score >= 7 ? 'bg-warning/10 text-warning' :
