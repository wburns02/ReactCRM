...
import { SurveyConditionalLogic, type ConditionalLogicRule } from './SurveyConditionalLogic.tsx';

// Types
export type QuestionType = 'rating' | 'scale' | 'text' | 'multiple_choice' | 'single_choice';
...
export interface SurveyBuilderProps {
  ...
  surveyName?: string;
  onSave?: (questions: SurveyQuestion[], name: string, saveAsTemplate: boolean) => void;
  onCancel?: () => void;
}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 p-4 cursor-move">
        ...
        <div className={cn('p-1.5 rounded', typeConfig?.type === question.type && 'bg-primary/10 text-primary')}>
          {typeConfig?.icon}
        </div>

        <span className="flex-1 font-medium text-text-primary truncate">{question.text || 'Untitled Question'}</span>
        ...
        <div className="px-4 pb-4 space-y-4 border-t border-border pt-4">
                ...
                <button
                  ...
                  onClick={() =>
                    onUpdate({
                      ...question,
                      type: qType.type,
                      options: qType.type === 'multiple_choice' || qType.type === 'single_choice' ? question.options || ['Option 1', 'Option 2'] : undefined,
          ...
          {question.type === 'scale' && (
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-1">Min Value</label>
                <input
                  type="number"
                  value={question.scale_min || 0}
                  onChange={(e) => onUpdate({ ...question, scale_min: parseInt(e.target.value) })}
                  className="w-full px-3 py-2 border border-border rounded-lg bg-bg-primary text-text-primary focus:outline-none focus:ring-2 focus:ring-primary"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-1">Max Value</label>
                <input
                  type="number"
                  value={question.scale_max || 10}
                  onChange={(e) => onUpdate({ ...question, scale_max: parseInt(e.target.value) })}
                  className="w-full px-3 py-2 border border-border rounded-lg bg-bg-primary text-text-primary focus:outline-none focus:ring-2 focus:ring-primary"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-1">Min Label</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-1">Max Label</label>
                <input
                  type="text"
                  ...
                  onChange={(e) =>
                    onUpdate({
                      ...question,
                      scale_labels: { min: question.scale_labels?.min || '', max: e.target.value },
                    })
                  }
                  className="w-full px-3 py-2 border border-border rounded-lg bg-bg-primary text-text-primary focus:outline-none focus:ring-2 focus:ring-primary"
              ...
              <div className="space-y-2">
                {(question.options || []).map((option, optIndex) => (
                  <div key={optIndex} className="flex items-center gap-2">
                    <span className="text-text-muted text-sm w-6">{optIndex + 1}.</span>
                    <input
                      type="text"
                      value={option}
          ...
          {question.type === 'text' && (
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-text-secondary mb-1">Max Length</label>
                <input
                  type="number"
                  value={question.max_length || ''}
                  onChange={(e) => onUpdate({ ...question, max_length: e.target.value ? parseInt(e.target.value) : undefined })}
                  className="w-full px-3 py-2 border border-border rounded-lg bg-bg-primary text-text-primary focus:outline-none focus:ring-2 focus:ring-primary"
          ...
          <div className="flex items-center justify-between pt-2 border-t border-border">
            ...
            >
              ...
              Conditional Logic
              {question.conditional_logic?.length ? ` (${question.conditional_logic.length})` : ''}
            </button>
          </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-bg-card rounded-xl shadow-xl w-full max-w-2xl mx-4 p-6 max-h-[80vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-text-primary">Start from Template</h2>
        ...
        <div className="grid gap-4">
          {DEFAULT_TEMPLATES.map((template) => (
            ...
            >
              ...
              <p className="text-xs text-text-muted">{template.questions.length} questions</p>
            </button>
          ))}
...
export function SurveyBuilder({
  ...
  const [showTemplateSelector, setShowTemplateSelector] = useState(initialQuestions.length === 0);
  ...
  const handleAddQuestion = useCallback((type: QuestionType = 'text') => {
    const newQuestion: SurveyQuestion = {
      ...
      text: '',
      required: false,
      options: type === 'multiple_choice' || type === 'single_choice' ? ['Option 1', 'Option 2'] : undefined,
  ...
  // Update question
  const handleUpdateQuestion = useCallback((index: number, question: SurveyQuestion) => {
    setQuestions((prev) => {
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-text-muted">
                  {questions.length} question{questions.length !== 1 ? 's' : ''} | {surveyType.toUpperCase()}
                </p>
              </div>
            </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="grid lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3 space-y-4">
            {questions.length === 0 ? (
              <div className="bg-bg-card rounded-xl border border-dashed border-border p-12 text-center">
                ...
                <p className="text-text-muted mb-4">Start by adding your first question or use a template</p>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-bg-card rounded-xl border border-border p-4">
              ...
              <div className="space-y-2">
                {QUESTION_TYPES.map((qType) => (
                  ...
                  >
                    <div className="p-1.5 rounded bg-bg-hover">{qType.icon}</div>
                    <div className="text-left">
                      <p className="text-sm font-medium">{qType.label}</p>
                      <p className="text-xs text-text-muted">{qType.description}</p>
                    </div>
                  </button>
                ))}
              </div>
            ...
            {/* Save Options */}
            <div className="bg-bg-card rounded-xl border border-border p-4">
              <h3 className="font-medium text-text-primary mb-3">Save Options</h3>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  ...
                  className="w-4 h-4 rounded border-border text-primary focus:ring-primary"
                />
                <span className="text-sm text-text-secondary">Save as template</span>
              </label>
              <p className="text-xs text-text-muted mt-2">Save this survey as a reusable template for future surveys</p>
            </div>

            {/* Stats */}
            <div className="bg-bg-card rounded-xl border border-border p-4">
              <h3 className="font-medium text-text-primary mb-3">Survey Stats</h3>
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-text-primary">{questions.filter((q) => q.required).length}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-text-muted">With Logic</span>
                  <span className="font-medium text-text-primary">
                    {questions.filter((q) => q.conditional_logic?.length).length}
                  </span>
                </div>
              </div>
            </div>
