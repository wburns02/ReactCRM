...
const STEP_TYPE_CONFIG: Record<string, { label: string; icon: string; color: string; bgColor: string; borderColor: string }> = {
  email: { label: 'Email', icon: '📧', color: 'text-blue-600 dark:text-blue-400', bgColor: 'bg-blue-50 dark:bg-blue-900/30', borderColor: 'border-blue-300 dark:border-blue-700' },
  task: { label: 'Task', icon: '✅', color: 'text-green-600 dark:text-green-400', bgColor: 'bg-green-50 dark:bg-green-900/30', borderColor: 'border-green-300 dark:border-green-700' },
  wait: { label: 'Wait', icon: '⏳', color: 'text-amber-600 dark:text-amber-400', bgColor: 'bg-amber-50 dark:bg-amber-900/30', borderColor: 'border-amber-300 dark:border-amber-700' },
  condition: { label: 'Condition', icon: '🔀', color: 'text-purple-600 dark:text-purple-400', bgColor: 'bg-purple-50 dark:bg-purple-900/30', borderColor: 'border-purple-300 dark:border-purple-700' },
  webhook: { label: 'Webhook', icon: '🔗', color: 'text-gray-600 dark:text-gray-400', bgColor: 'bg-gray-50 dark:bg-gray-900/30', borderColor: 'border-gray-300 dark:border-gray-700' },
  human_touchpoint: { label: 'Human Touch', icon: '👤', color: 'text-cyan-600 dark:text-cyan-400', bgColor: 'bg-cyan-50 dark:bg-cyan-900/30', borderColor: 'border-cyan-300 dark:border-cyan-700' },
  in_app_message: { label: 'In-App', icon: '💬', color: 'text-indigo-600 dark:text-indigo-400', bgColor: 'bg-indigo-50 dark:bg-indigo-900/30', borderColor: 'border-indigo-300 dark:border-indigo-700' },
  sms: { label: 'SMS', icon: '📱', color: 'text-teal-600 dark:text-teal-400', bgColor: 'bg-teal-50 dark:bg-teal-900/30', borderColor: 'border-teal-300 dark:border-teal-700' },
  notification: { label: 'Notification', icon: '🔔', color: 'text-orange-600 dark:text-orange-400', bgColor: 'bg-orange-50 dark:bg-orange-900/30', borderColor: 'border-orange-300 dark:border-orange-700' },
  update_field: { label: 'Update Field', icon: '📝', color: 'text-slate-600 dark:text-slate-400', bgColor: 'bg-slate-50 dark:bg-slate-900/30', borderColor: 'border-slate-300 dark:border-slate-700' },
  add_tag: { label: 'Add Tag', icon: '🏷️', color: 'text-pink-600 dark:text-pink-400', bgColor: 'bg-pink-50 dark:bg-pink-900/30', borderColor: 'border-pink-300 dark:border-pink-700' },
  enroll_journey: { label: 'Enroll Journey', icon: '🗺️', color: 'text-violet-600 dark:text-violet-400', bgColor: 'bg-violet-50 dark:bg-violet-900/30', borderColor: 'border-violet-300 dark:border-violet-700' },
  trigger_playbook: { label: 'Trigger Playbook', icon: '📘', color: 'text-rose-600 dark:text-rose-400', bgColor: 'bg-rose-50 dark:bg-rose-900/30', borderColor: 'border-rose-300 dark:border-rose-700' },
  health_check: { label: 'Health Check', icon: '💓', color: 'text-red-600 dark:text-red-400', bgColor: 'bg-red-50 dark:bg-red-900/30', borderColor: 'border-red-300 dark:border-red-700' },
  custom: { label: 'Custom', icon: '⚙️', color: 'text-gray-600 dark:text-gray-400', bgColor: 'bg-gray-50 dark:bg-gray-900/30', borderColor: 'border-gray-300 dark:border-gray-700' },
};

type ViewMode = 'flow' | 'list' | 'analytics';

// Visual Flow Node Component
function FlowNode({ step, index, isLast, isCondition }: { step: JourneyStep; index: number; isLast: boolean; isCondition?: boolean }) {
  const [showDetails, setShowDetails] = useState(false);
  const stepConfig = STEP_TYPE_CONFIG[step.step_type] || STEP_TYPE_CONFIG.custom;
    ...
    <div className="relative flex flex-col items-center">
      ...
      <button
        ...
        className={cn(
          ...
          showDetails && 'ring-2 ring-primary ring-offset-2 dark:ring-offset-gray-900'
        )}
      >
        {/* Step number badge */}
        <div className={cn(
          ...
          'bg-gradient-to-br',
          stepConfig.color.includes('blue') ? 'from-blue-500 to-blue-600' :
          stepConfig.color.includes('green') ? 'from-green-500 to-green-600' :
          stepConfig.color.includes('amber') ? 'from-amber-500 to-amber-600' :
          stepConfig.color.includes('purple') ? 'from-purple-500 to-purple-600' :
          stepConfig.color.includes('cyan') ? 'from-cyan-500 to-cyan-600' :
          stepConfig.color.includes('pink') ? 'from-pink-500 to-pink-600' :
          stepConfig.color.includes('teal') ? 'from-teal-500 to-teal-600' :
          stepConfig.color.includes('orange') ? 'from-orange-500 to-orange-600' :
          stepConfig.color.includes('red') ? 'from-red-500 to-red-600' :
        ...
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className={cn('text-xs font-semibold uppercase tracking-wide', stepConfig.color)}>
            {stepConfig.label}
          </span>
        </div>
      ...
      {showDetails && (
        <div className={cn(
          ...
          {step.condition_rules != null && (
            <div className="mb-3">
              <h5 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Conditions</h5>
          ...
          {step.action_config != null && (
            <div>
              <h5 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Configuration</h5>
  ...
  return (
    <div className="flex flex-col items-center py-6 px-4">
      ...
      <div className="flex items-center gap-2 px-4 py-2 bg-green-100 dark:bg-green-900/30 rounded-full border-2 border-green-400 dark:border-green-600 mb-4">
        ...
        <span className="text-sm font-semibold text-green-700 dark:text-green-400">START</span>
      </div>

      <div className="w-0.5 h-8 bg-gradient-to-b from-green-400 to-gray-400 dark:from-green-600 dark:to-gray-500" />
      <svg className="w-4 h-4 text-gray-400 -my-1" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 16l-6-6h12l-6 6z" />
      </svg>
      ...
      <div className="flex items-center gap-2 px-4 py-2 bg-red-100 dark:bg-red-900/30 rounded-full border-2 border-red-400 dark:border-red-600 mt-4">
        ...
        <span className="text-sm font-semibold text-red-700 dark:text-red-400">END</span>
      </div>
    </div>
  );
}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-1.5 flex-wrap">
                  ...
                  <span className={cn('text-xs px-2.5 py-1 rounded-full bg-white/80 dark:bg-black/20 font-medium', stepConfig.color)}>
                    {stepConfig.label}
                  </span>
...
function AnalyticsView({ journey, steps }: { journey: Journey; steps: JourneyStep[] }) {
  ...
  const totalWaitHours = useMemo(() => {
    return steps.reduce((total, step) => total + (step.wait_duration_hours || 0), 0);
  }, [steps]);
    ...
    <div className="space-y-6 py-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/30 dark:to-blue-800/30 rounded-xl p-4 border border-blue-200 dark:border-blue-700">
          <p className="text-xs text-blue-600 dark:text-blue-400 font-medium uppercase tracking-wide">Total Steps</p>
          <p className="text-3xl font-bold text-blue-700 dark:text-blue-300 mt-1">{steps.length}</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-amber-50 to-amber-100 dark:from-amber-900/30 dark:to-amber-800/30 rounded-xl p-4 border border-amber-200 dark:border-amber-700">
          <p className="text-xs text-amber-600 dark:text-amber-400 font-medium uppercase tracking-wide">Journey Duration</p>
          <p className="text-3xl font-bold text-amber-700 dark:text-amber-300 mt-1">{totalWaitDays}d</p>
          <p className="text-xs text-amber-500 mt-1">{totalWaitHours} hours total</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/30 dark:to-green-800/30 rounded-xl p-4 border border-green-200 dark:border-green-700">
          <p className="text-xs text-green-600 dark:text-green-400 font-medium uppercase tracking-wide">Enrolled</p>
          <p className="text-3xl font-bold text-green-700 dark:text-green-300 mt-1">{journey.active_enrollments || journey.active_enrolled || 0}</p>
          <p className="text-xs text-green-500 mt-1">{journey.total_enrolled || 0} total</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/30 dark:to-purple-800/30 rounded-xl p-4 border border-purple-200 dark:border-purple-700">
          <p className="text-xs text-purple-600 dark:text-purple-400 font-medium uppercase tracking-wide">Conversion</p>
          <p className={cn(
            'text-3xl font-bold mt-1',
            (journey.conversion_rate || 0) >= 50 ? 'text-green-600 dark:text-green-400' :
            (journey.conversion_rate || 0) >= 25 ? 'text-amber-600 dark:text-amber-400' : 'text-red-600 dark:text-red-400'
          )}>
            {(journey.conversion_rate || 0).toFixed(1)}%
          </p>
          <p className="text-xs text-purple-500 mt-1">{journey.completed_count || 0} completed</p>
        </div>
      </div>

      {/* Step Type Distribution */}
      <div className="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700">
        <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">Step Type Distribution</h4>
        <div className="space-y-3">
          {stepTypeDistribution.map(([type, count]) => {
            ...
            return (
              <div key={type} className="flex items-center gap-3">
                ...
                <span className={cn('text-sm font-medium w-28', config.color)}>{config.label}</span>
                <div className="flex-1 h-3 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                  <div
                    className={cn('h-full rounded-full transition-all', config.bgColor, 'bg-opacity-100 dark:bg-opacity-100')}
                    style={{ width: `${percentage}%`, backgroundColor: config.color.includes('blue') ? '#3b82f6' : config.color.includes('green') ? '#22c55e' : config.color.includes('amber') ? '#f59e0b' : config.color.includes('purple') ? '#a855f7' : config.color.includes('cyan') ? '#06b6d4' : config.color.includes('pink') ? '#ec4899' : config.color.includes('teal') ? '#14b8a6' : config.color.includes('orange') ? '#f97316' : config.color.includes('red') ? '#ef4444' : '#6b7280' }}
                  />
                </div>
                <span className="text-sm text-gray-600 dark:text-gray-400 w-16 text-right">{count} ({percentage}%)</span>
              </div>
            );
          })}
        </div>
      ...
      {/* Journey Timeline */}
      <div className="bg-white dark:bg-gray-800 rounded-xl p-5 border border-gray-200 dark:border-gray-700">
        <h4 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-4">Journey Timeline</h4>
        <div className="relative">
          <div className="absolute left-0 top-0 bottom-0 w-0.5 bg-gray-200 dark:bg-gray-700" />
          <div className="space-y-4 pl-6">
            {steps.map((step, index) => {
              const config = STEP_TYPE_CONFIG[step.step_type] || STEP_TYPE_CONFIG.custom;
              const cumulativeHours = steps.slice(0, index + 1).reduce((sum, s) => sum + (s.wait_duration_hours || 0), 0);

              return (
                <div key={step.id || index} className="relative">
                  <div className={cn(
                    'absolute -left-6 top-1 w-3 h-3 rounded-full border-2 border-white dark:border-gray-800',
                    step.step_type === 'condition' ? 'bg-purple-500' :
                    step.step_type === 'wait' ? 'bg-amber-500' :
                    'bg-blue-500'
                  )} />
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-700 dark:text-gray-300 font-medium">{step.name}</span>
                  </div>
                </div>
              );
            })}
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-6 sm:pt-12 px-4">
      ...
      <div className="relative bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
          ...
          <div className="relative p-6">
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className={cn(
                    ...
                    <div className={cn('w-2 h-2 rounded-full', statusConfig.dotColor, journey.status === 'active' && 'animate-pulse')} />
                    {statusConfig.label}
                  </span>
                </div>
                <h2 className="text-2xl font-bold text-white">{journey.name}</h2>
                {journey.description && (
                  <p className="text-sm text-white/80 mt-1 max-w-xl">{journey.description}</p>
                )}
              </div>
              <button
                onClick={onClose}
            ...
            <div className="flex items-center gap-4 mt-4 flex-wrap">
              ...
              <div className="flex items-center gap-1.5 text-sm text-white/90">
                ...
                <span className="font-semibold">{journey.active_enrollments || journey.active_enrolled || 0}</span> enrolled
              </div>
              {journey.conversion_rate !== null && journey.conversion_rate !== undefined && (
                <div className="flex items-center gap-1.5 text-sm text-white/90">
                  ...
                  <span className="font-semibold">{journey.conversion_rate.toFixed(1)}%</span> conversion
                </div>
              )}
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gray-50 dark:bg-gray-900/50">
          ...
          {viewMode === 'analytics' && <AnalyticsView journey={journey} steps={steps} />}
        </div>
