...
): "email" | "delay" | "condition" | "action" | "in_app" {
...
function transformCampaign(apiCampaign: Record<string, unknown>): Campaign {
  const steps = Array.isArray(apiCampaign.steps)
    ? apiCampaign.steps.map((step: Record<string, unknown>) => ({
        ...
        delay_days:
          ((step.delay_days as number) || 0) +
          ((step.delay_hours as number) || 0) / 24,
        subject: step.subject as string | undefined,
        content: step.content as string | undefined,
        condition: step.condition_rules
          ? JSON.stringify(step.condition_rules)
          : undefined,
      }))
    : [];
  ...
  return {
    ...
    type: mapCampaignType(
      (apiCampaign.campaign_type as string) ||
        (apiCampaign.primary_channel as string),
    ),
...
function ABTestResults({ test }: { test: ABTest }) {
  ...
  const convRateA =
    totalA > 0 ? (test.variant_a.conversions / totalA) * 100 : 0;
  const convRateB =
    totalB > 0 ? (test.variant_b.conversions / totalB) * 100 : 0;
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        >
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-text-muted">
              {test.variant_a.conversions}/{test.variant_a.sent}
            </span>
          </div>
        </div>
        ...
        >
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-text-muted">
              {test.variant_b.conversions}/{test.variant_b.sent}
            </span>
          </div>
        </div>
      </div>
...
export function NurtureCampaignManager() {
  ...
  const demoCampaigns: Campaign[] = [
    {
      ...
      status: "active",
      goal: "onboarding",
      description: "Automated 14-day welcome sequence",
    ...
    },
    {
      ...
      target_segment_name: "Active Contracts",
      sent_count: 1243,
      open_rate: 52.3,
      click_rate: 18.7,
      conversion_rate: 8.4,
      steps: [
        {
          ...
          name: "Service Due",
          type: "email",
          subject: "Service Coming Up",
        },
      ],
      ab_test: {
        id: 1,
        variant_a: { name: "Urgency", sent: 621, conversions: 58 },
        variant_b: { name: "Friendly", sent: 622, conversions: 72 },
        winner: "b",
        confidence: 94,
      },
    ...
    },
    {
      ...
      status: "scheduled",
      goal: "expansion",
      description: "Proactive renewal outreach",
      target_segment_name: "Expiring Contracts",
      sent_count: 0,
      steps: [
        {
          id: 1,
    ...
    },
    {
      ...
      status: "active",
      goal: "adoption",
      description: "Drive feature adoption",
      target_segment_name: "Low Usage",
      sent_count: 234,
      open_rate: 45.2,
      click_rate: 22.8,
      conversion_rate: 15.3,
      steps: [{ id: 1, order: 1, name: "Feature Tip", type: "in_app" }],
      created_at: "2025-12-20",
    },
    {
      id: 6,
      name: "Emergency Prep Education",
      type: "email",
      status: "draft",
      goal: "education",
      description: "Educational content",
      target_segment_name: "All Residential",
      sent_count: 0,
      steps: [
        {
          id: 1,
  ...
  const apiCampaigns: Campaign[] = campaignsData?.items
    ? campaignsData.items.map((item: Record<string, unknown>) =>
        transformCampaign(item),
      )
    : [];
  ...
  const campaigns =
    filter === "all"
      ? allCampaigns
      : allCampaigns.filter((c) => c.goal === filter);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-text-muted">
            Automated customer engagement sequences
          </p>
        </div>
