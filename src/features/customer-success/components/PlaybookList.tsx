      ...
      <div className="flex items-center gap-4 text-sm flex-wrap">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-text-muted">
            {playbook.steps?.length ?? playbook.step_count ?? 0} steps
          </span>
        </div>
        ...
        <div className="mt-3 pt-3 border-t border-border">
          <p className="text-xs text-text-muted">
            ...
            {playbook.trigger_health_threshold && (
              <span>
                {" "}
                (health {"<"} {playbook.trigger_health_threshold})
              </span>
            )}
          </p>
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {filteredPlaybooks.map((playbook) => (
            <PlaybookCard
              ...
              onEdit={
                onEditPlaybook ? () => onEditPlaybook(playbook) : undefined
              }
              onDelete={
                onDeletePlaybook ? () => onDeletePlaybook(playbook) : undefined
              }
              onTrigger={
                onTriggerPlaybook
