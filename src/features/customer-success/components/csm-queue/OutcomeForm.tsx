...
import type { CSMQueueTask, CSMTaskType, CSMTaskOutcomeFormData, CSMOutcomeType, CSMObjectiveAchieved, CSMSentiment } from '../../../../api/types/customerSuccess';
...
const outcomeTypeOptions: { value: CSMOutcomeType; label: string; description: string }[] = [
  { value: 'connected', label: 'Connected', description: 'Spoke with customer' },
  { value: 'voicemail', label: 'Voicemail', description: 'Left voicemail message' },
  { value: 'no_answer', label: 'No Answer', description: 'No response received' },
  { value: 'rescheduled', label: 'Rescheduled', description: 'Moved to later date' },
  { value: 'completed', label: 'Completed', description: 'Task fully completed' },
  { value: 'escalated', label: 'Escalated', description: 'Escalated to manager' },
];
...
const sentimentOptions: { value: CSMSentiment; label: string; emoji: string }[] = [
...
export function OutcomeForm({ taskType, onSubmit, onCancel, isPending }: OutcomeFormProps) {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.outcome_type === 'connected' && !formData.objective_achieved) {
      newErrors.objective_achieved = 'Please indicate if the objective was achieved';
    }
    ...
    if (!formData.notes || formData.notes.trim().length < 10) {
      newErrors.notes = 'Please provide detailed notes (at least 10 characters)';
    }

    // Validate required quality gates
    qualityGates.forEach(gate => {
      if (gate.required && formData.quality_gate_responses?.[gate.id] === undefined) {
  ...
  const updateGateResponse = (gateId: string, value: string | boolean | number) => {
    setFormData(prev => ({
      ...prev,
      quality_gate_responses: {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
          {outcomeTypeOptions.map(option => (
            <button
              key={option.value}
              type="button"
              onClick={() => setFormData(prev => ({ ...prev, outcome_type: option.value }))}
              className={`
        ...
        {errors.outcome_type && <p className="text-red-400 text-sm mt-1">{errors.outcome_type}</p>}
      </div>
      ...
      {formData.outcome_type === 'connected' && (
        <>
          ...
          <div>
            <label className="block text-sm font-medium text-text-secondary mb-2">
              Was the objective achieved? <span className="text-red-400">*</span>
            </label>
              ...
              {objectiveOptions.map(option => (
                <label key={option.value} className="flex items-center gap-3 cursor-pointer">
                  <input
                    ...
                    onChange={() => setFormData(prev => ({ ...prev, objective_achieved: option.value }))}
            ...
            {errors.objective_achieved && <p className="text-red-400 text-sm mt-1">{errors.objective_achieved}</p>}
          </div>

          {/* Blocker Reason (if not achieved) */}
          {(formData.objective_achieved === 'no' || formData.objective_achieved === 'partial') && (
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-2">
          ...
          <div>
            ...
            <div className="flex gap-2">
              {sentimentOptions.map(option => (
                <button
                  key={option.value}
                  type="button"
                  onClick={() => setFormData(prev => ({ ...prev, sentiment: option.value }))}
                  className={`
                    flex-1 flex items-center justify-center gap-2 p-3 rounded-lg border transition-all
                    ${formData.sentiment === option.value
                      ? option.value === 'positive' ? 'border-green-500 bg-green-500/10 text-green-400'
                        : option.value === 'neutral' ? 'border-gray-500 bg-gray-500/10 text-gray-400'
                        : option.value === 'frustrated' ? 'border-yellow-500 bg-yellow-500/10 text-yellow-400'
            ...
            {errors.sentiment && <p className="text-red-400 text-sm mt-1">{errors.sentiment}</p>}
          </div>
        </>
      )}
          ...
          <div className="space-y-4">
            {qualityGates.map(gate => (
              <div key={gate.id}>
                <label className="block text-sm font-medium text-text-secondary mb-2">
                  {gate.question}
                  {gate.required && <span className="text-red-400 ml-1">*</span>}
                </label>
                  ...
                  <div className="flex gap-4">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="radio"
                        name={`gate_${gate.id}`}
                        checked={formData.quality_gate_responses?.[gate.id] === true}
                    ...
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        type="radio"
                        name={`gate_${gate.id}`}
                        checked={formData.quality_gate_responses?.[gate.id] === false}
                ...
                {gate.gate_type === 'select' && gate.options && (
                  <select
                    value={formData.quality_gate_responses?.[gate.id] as string || ''}
                    onChange={e => updateGateResponse(gate.id, e.target.value)}
                    ...
                    {[1, 2, 3, 4, 5].map(rating => (
                      <button
                        ...
                        className={`
                          ...
                          ${formData.quality_gate_responses?.[gate.id] === rating
                ...
                {errors[`gate_${gate.id}`] && (
                  <p className="text-red-400 text-sm mt-1">{errors[`gate_${gate.id}`]}</p>
                )}
              </div>
            ))}
          </div>
      ...
      <div>
        <label className="flex items-center gap-3 cursor-pointer">
          <input
            ...
            onChange={e => setFormData(prev => ({ ...prev, next_action_required: e.target.checked }))}
            className="w-5 h-5 text-primary bg-bg-primary border-border rounded focus:ring-primary"
          />
          <span className="text-text-primary font-medium">Follow-up action required</span>
        </label>
          ...
          <div className="mt-4 pl-8 space-y-3">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2 rounded-lg bg-bg-primary border border-border text-text-primary focus:outline-none focus:ring-2 focus:ring-primary"
              />
              {errors.next_action_date && <p className="text-red-400 text-sm mt-1">{errors.next_action_date}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-text-secondary mb-2">
                Follow-up Notes
              ...
              <textarea
                ...
                onChange={e => setFormData(prev => ({ ...prev, next_action_notes: e.target.value }))}
