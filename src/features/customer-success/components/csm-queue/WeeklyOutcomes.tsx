...
function MetricCard({ title, actual, goal, prefix = '', suffix = '', icon, color }: MetricCardProps) {
  const progress = goal ? Math.min((actual / goal) * 100, 100) : 100;
  const isOnTrack = goal ? actual >= goal * 0.7 : true;

  const colorClasses = {
    green: { bg: 'bg-green-500/20', text: 'text-green-400', progressBg: 'bg-green-500' },
    blue: { bg: 'bg-blue-500/20', text: 'text-blue-400', progressBg: 'bg-blue-500' },
    purple: { bg: 'bg-purple-500/20', text: 'text-purple-400', progressBg: 'bg-purple-500' },
    orange: { bg: 'bg-orange-500/20', text: 'text-orange-400', progressBg: 'bg-orange-500' },
    yellow: { bg: 'bg-yellow-500/20', text: 'text-yellow-400', progressBg: 'bg-yellow-500' },
  };
  ...
  return (
    <div className="bg-bg-secondary rounded-lg border border-border p-4">
      <div className="flex items-start justify-between mb-3">
        ...
        {goal && (
          <span className={`text-xs px-2 py-1 rounded ${isOnTrack ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
      ...
      {goal && (
        <div className="mt-3">
          ...
          <span className="text-xs text-text-muted mt-1 block">{progress.toFixed(0)}% of goal</span>
        </div>
      )}
    </div>
  );
  ...
  if (!outcomes) {
    return (
      <div className="text-center py-12 bg-bg-secondary rounded-lg border border-border">
        ...
        <h3 className="text-lg font-semibold text-text-primary mb-2">No Outcome Data</h3>
        <p className="text-text-muted">Complete tasks to see your weekly outcomes.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-text-primary">Weekly Outcomes</h2>
          <p className="text-sm text-text-muted">
            {new Date(outcomes.period_start).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} -
            {new Date(outcomes.period_end).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
          </p>
        </div>
        <div className={`
        ...
        `}>
          <svg className={`w-5 h-5 ${outcomes.trend_vs_last_week >= 0 ? '' : 'rotate-180'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
          </svg>
          <span className="font-medium">
            {outcomes.trend_vs_last_week >= 0 ? '+' : ''}{outcomes.trend_vs_last_week}% vs last week
          </span>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <MetricCard
            ...
            prefix={formatCurrency(outcomes.outcomes.renewals_secured.amount).replace(/[0-9,.]+/, '')}
          ...
          <MetricCard
            ...
            prefix={formatCurrency(outcomes.outcomes.expansion_pipeline.amount).replace(/[0-9,.]+/, '')}
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-bg-secondary rounded-lg border border-border p-4 text-center">
            <div className="text-2xl font-bold text-text-primary">{outcomes.activity.calls_made}</div>
            <div className="text-sm text-text-muted">Calls Made</div>
          </div>
          <div className="bg-bg-secondary rounded-lg border border-border p-4 text-center">
            <div className="text-2xl font-bold text-text-primary">{outcomes.activity.emails_sent}</div>
            <div className="text-sm text-text-muted">Emails Sent</div>
          </div>
          <div className="bg-bg-secondary rounded-lg border border-border p-4 text-center">
            <div className="text-2xl font-bold text-text-primary">{outcomes.activity.tasks_completed}</div>
        ...
        <div className="flex items-center gap-8">
          <div className="flex-1">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="font-bold text-primary">{outcomes.comparison.your_score}</span>
            </div>
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="font-bold text-text-secondary">{outcomes.comparison.team_average}</span>
            </div>
            <div className="h-3 bg-bg-primary rounded-full overflow-hidden">
              <div
          ...
          <div className="text-center px-6 py-3 bg-bg-primary rounded-lg">
            <div className="text-3xl font-bold text-primary">{outcomes.comparison.percentile}th</div>
