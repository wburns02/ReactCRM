...
import { useCSMTaskQueue, useCSMWeeklyOutcomes, useStartCSMTask, useCompleteCSMTask } from '../../../../api/hooks/useCustomerSuccess';
import type { CSMQueueTask, CSMQueueFilters, CSMTaskOutcomeFormData } from '../../../../api/types/customerSuccess';
...
export function CSMQueueTab() {
  ...
  // API queries
  const { data: queueData, isLoading: isQueueLoading } = useCSMTaskQueue(filters);
  const { data: outcomesData, isLoading: isOutcomesLoading } = useCSMWeeklyOutcomes();
  ...
  const tasks = useMemo(() => {
    ...
    if (filters.category) {
      const categoryTaskSlugs = demoTaskTypes
        ...
        .map(tt => tt.slug);
      filteredTasks = filteredTasks.filter(t => categoryTaskSlugs.includes(t.task_type_slug || ''));
    }
  ...
  const selectedTaskType = selectedTask
    ? demoTaskTypes.find(tt => tt.id === selectedTask.task_type_id || tt.slug === selectedTask.task_type_slug)
    : undefined;
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-text-primary">CSM Task Queue</h1>
          <p className="text-text-muted">Your prioritized work queue - focus on outcomes, not activities</p>
        </div>
      ...
      {viewMode === 'queue' ? (
        <div className="flex-1 flex gap-6 min-h-0">
          {/* Left: Queue List */}
          <div className={`flex flex-col min-h-0 ${selectedTask ? 'w-1/2' : 'w-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto">
          <WeeklyOutcomes outcomes={weeklyOutcomes} isLoading={isOutcomesLoading} />
        </div>
      )}
    </div>
  );
