  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-xs font-semibold ${config.bgColor} ${config.textColor}`}>
          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={config.icon} />
          </svg>
          {config.badge}
        </div>
        <span className="text-xs text-text-muted">Score: {task.priority_score}</span>
      </div>

      {/* Task Type */}
      <h3 className="font-semibold text-text-primary mb-2">{task.task_type_name}</h3>

      {/* Customer Info */}
      <div className="mb-3">
        <div className="flex items-center gap-2 mb-1">
          <span className="font-medium text-text-primary">{task.customer_name}</span>
          {task.customer_tier && (
      ...
      {/* Due Date */}
      {dueInfo && (
        <div className={`text-sm mb-3 ${dueInfo.isOverdue ? 'text-red-400 font-medium' : 'text-text-muted'}`}>
          {dueInfo.isOverdue && (
