...
export function PlaybookPanel({ playbook, customerName = '[name]' }: PlaybookPanelProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {playbook.key_questions && playbook.key_questions.length > 0 && (
        <div className="bg-bg-secondary rounded-lg p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-semibold text-text-primary">Key Questions to Ask</h3>
          </div>
      ...
      {/* Objection Handlers */}
      {playbook.objection_handlers && playbook.objection_handlers.length > 0 && (
        <div className="bg-bg-secondary rounded-lg p-4">
          <div className="flex items-center gap-2 mb-3">
            <svg className="w-5 h-5 text-text-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
            <h3 className="font-semibold text-text-primary">Objection Handlers</h3>
          </div>
          <div className="space-y-2">
            {playbook.objection_handlers.map((handler) => (
              ...
              >
                <button
                  onClick={() => setExpandedObjection(expandedObjection === handler.id ? null : handler.id)}
                ...
                {expandedObjection === handler.id && (
                  <div className="px-3 pb-3">
                    <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-3">
                      <span className="text-xs text-green-400 font-medium mb-1 block">Your Response:</span>
                      <p className="text-text-primary">{processScript(handler.response)}</p>
                    </div>
                  </div>
                )}
              </div>
      ...
      <div className="bg-bg-secondary rounded-lg p-4">
        ...
        <p className="text-text-muted">{processScript(playbook.success_criteria)}</p>
      </div>
      ...
      <div className="flex items-center gap-2 text-sm text-text-muted">
        ...
        <span>Estimated duration: {playbook.estimated_duration_minutes} minutes</span>
      </div>
    </div>
  );
}
