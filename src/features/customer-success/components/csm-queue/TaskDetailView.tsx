...
import type { CSMQueueTask, CSMPlaybook, CSMTaskType, Touchpoint } from '../../../../api/types/customerSuccess';
...
export function TaskDetailView({
  ...
  const [activeTab, setActiveTab] = useState<'playbook' | 'context' | 'history' | 'outcome'>('playbook');
  ...
  return (
    <div className="h-full flex flex-col bg-bg-card rounded-lg border border-border overflow-hidden">
      ...
      <div className="flex items-center justify-between p-4 border-b border-border">
        <div>
          <h2 className="text-lg font-semibold text-text-primary">{task.task_type_name}</h2>
          <p className="text-sm text-text-muted">
            {customer?.name || task.customer_name} - {formatCurrency(customer?.arr || task.customer_arr)}
          </p>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {activeTab === 'playbook' && playbook && (
          <PlaybookPanel playbook={playbook} customerName={customer?.name || task.customer_name || 'Customer'} />
        )}
        {activeTab === 'playbook' && !playbook && (
        ...
        {activeTab === 'context' && (
          <div className="space-y-6">
            ...
            <div className="bg-bg-secondary rounded-lg p-4">
              <h3 className="font-semibold text-text-primary mb-4">Customer Overview</h3>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="text-xs text-text-muted block mb-1">Health Score</span>
                  <div className="flex items-center gap-2">
                    <span className={`text-lg font-bold ${
                      (customer?.health_score || task.customer_health_score || 0) >= 70 ? 'text-green-400' :
                      (customer?.health_score || task.customer_health_score || 0) >= 40 ? 'text-yellow-400' : 'text-red-400'
                    }`}>
                      {customer?.health_score || task.customer_health_score || '-'}
                    </span>
                    {customer?.health_status && (
                      <span className={`text-xs px-2 py-0.5 rounded ${healthStatusColors[customer.health_status as keyof typeof healthStatusColors] || ''}`}>
                ...
                <div>
                  <span className="text-xs text-text-muted block mb-1">ARR</span>
                  <span className="text-lg font-bold text-text-primary">
                    {formatCurrency(customer?.arr || task.customer_arr)}
                  </span>
                </div>
                <div>
                  <span className="text-xs text-text-muted block mb-1">Tier</span>
                  <span className="text-sm text-text-primary capitalize">
                    {(customer?.tier || task.customer_tier || '-').replace('_', ' ')}
                  </span>
                </div>
                <div>
                  <span className="text-xs text-text-muted block mb-1">Churn Probability</span>
                  <span className={`text-sm font-medium ${
                    (customer?.churn_probability || 0) > 0.5 ? 'text-red-400' :
                    (customer?.churn_probability || 0) > 0.3 ? 'text-yellow-400' : 'text-green-400'
                  }`}>
                    {customer?.churn_probability ? `${(customer.churn_probability * 100).toFixed(0)}%` : '-'}
                  </span>
                </div>
                <div>
                  <span className="text-xs text-text-muted block mb-1">Renewal Date</span>
                  <span className="text-sm text-text-primary">
                    {customer?.renewal_date || task.customer_renewal_date
                      ? new Date(customer?.renewal_date || task.customer_renewal_date || '').toLocaleDateString()
                      : '-'}
                  </span>
                </div>
                <div>
                  <span className="text-xs text-text-muted block mb-1">Last Contact</span>
                  <span className="text-sm text-text-primary">
                    {customer?.days_since_last_contact !== null && customer?.days_since_last_contact !== undefined
            ...
            <div className="bg-bg-secondary rounded-lg p-4">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <a href={`mailto:${customer?.email || task.customer_email}`} className="text-primary hover:underline">
            ...
            {customer?.contacts && customer.contacts.length > 0 && (
              <div className="bg-bg-secondary rounded-lg p-4">
                <h3 className="font-semibold text-text-primary mb-4">Key Contacts</h3>
                <div className="space-y-3">
                  {customer.contacts.map((contact, idx) => (
                    <div key={idx} className="flex items-center justify-between p-3 bg-bg-primary rounded-lg">
                      <div>
                        <div className="flex items-center gap-2">
          ...
          <div className="space-y-4">
            {interactionHistory && interactionHistory.length > 0 ? (
              interactionHistory.map(touchpoint => (
                <div key={touchpoint.id} className="bg-bg-secondary rounded-lg p-4">
                  ...
                  {touchpoint.subject && (
                    <h4 className="font-medium text-text-primary mb-1">{touchpoint.subject}</h4>
                  )}
                  {touchpoint.summary && (
                    <p className="text-sm text-text-muted">{touchpoint.summary}</p>
                  )}
                  {touchpoint.contact_name && (
                    <p className="text-xs text-text-muted mt-2">
                      Contact: {touchpoint.contact_name} ({touchpoint.contact_role})
                    </p>
                  )}
                </div>
              ))
