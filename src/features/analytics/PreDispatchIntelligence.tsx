...
export function PreDispatchIntelligence({
  ...
  const { data: predictions, isLoading, error } = useAIWorkOrderPredictions(workOrderId);
    ...
    <Card className={cn(className)}>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Pre-Dispatch Intelligence
          <Badge variant="primary" className="text-xs">FTFR Boost</Badge>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        {predictions.recommended_technician && (
          <div className="p-4 bg-gradient-to-r from-primary/10 to-purple-500/10 rounded-lg border border-primary/20">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-medium text-text-secondary">Recommended Technician</p>
              <Badge variant="success" className="text-xs">Best Match</Badge>
            </div>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-primary/20 flex items-center justify-center text-primary font-bold">
                {predictions.recommended_technician.name.split(' ').map(n => n[0]).join('')}
              </div>
              <div>
            ...
            <div className="space-y-2">
              {visibleParts.map((part, index) => (
                ...
                >
                  ...
                  <Badge
                    variant={part.probability >= 0.8 ? 'success' : part.probability >= 0.5 ? 'warning' : 'default'}
                    className="text-xs"
                  >
            ...
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-medium text-text-primary flex items-center gap-2">
                ...
                <Badge variant="primary" className="text-xs">
                  {predictions.similar_jobs.filter(j => j.success).length}/{predictions.similar_jobs.length} successful
                </Badge>
              </h4>
            </div>
              ...
              {visibleSimilarJobs.map((job, index) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <Badge variant={job.success ? 'success' : 'warning'} className="text-xs">
                      {job.success ? 'Fixed First Time' : 'Required Follow-up'}
                    </Badge>
                    <span className="text-xs text-text-muted">Job #{job.id}</span>
                  </div>
        ...
        <div className="p-3 bg-gradient-to-r from-success/10 to-primary/10 rounded-lg border border-success/20">
          <p className="text-sm text-text-secondary flex items-center gap-2">
            ...
            Using this intelligence can improve your First-Time Fix Rate by up to 15%
          </p>
        </div>
      </CardContent>
    </Card>
...
export function PreDispatchIntelligenceCompact({
  ...
  const { data: predictions, isLoading } = useAIWorkOrderPredictions(workOrderId);
