...
export const INSPECTION_STEPS: InspectionStep[] = [
  ...
  },
  {
    ...
    customInputs: [
      ...
      {
        id: "effluent_depth",
        label: "Effluent Chamber — Depth (inches)",
        type: "text",
        placeholder: "e.g., 8",
      },
    ],
  ...
  },
  {
    ...
    emoji: "🎛️",
    description: "Verify breakers, inspect for corrosion, check control panel & timer, test alarm, check air filter, and confirm pump operation.",
    hasPsiReading: true,
    detailedInstructions: [
      // Breakers first
      "Verify ALL breakers are in the ON position (including any dedicated to the septic system)",
      // Corrosion & connections
      "Inspect all metal components and connections for rust or corrosion (panel, conduits, wires, etc.)",
      "Check wire nuts (tight, no corrosion or exposed wire)",
      // Control panel inspection
      "Open control panel door carefully",
      "Check for corrosion or defects inside control panel",
      "Check control panel for leaks, loose wires, or visible damage",
      "Locate panel model and age (visible in/on control panel)",
      // Timer check
      "Note timer mechanism in control panel",
      "Verify timer setting: 15 minutes ON every 4 hours (6 cycles per day)",
      "Check that timer pins/dials/settings are correct",
      "Note current time position on timer for verification",
      "Confirm standard 24-hour cycle: 15 min every 4 hours when triggered",
      // Alarm test
      "Test alarm silence/reset function",
      "Verify all indicator lights function",
      "Check for warning lights (on panel)",
      "Test alarm light — should illuminate when activated",
      "Test buzzer — should sound when alarm activated",
      "Replace alarm bulb if burned out (note: replacement should ensure homeowner can see/hear it)",
      // Sealant & conduits
      "Apply silicone sealant where needed (e.g., around entries, conduits, or degraded seals to prevent water intrusion)",
      "Check conduit entry points for water intrusion or damage; note any parts needing replacement",
      // CHECK AIR wiring & air filter
      "Verify CHECK AIR wiring integrity from house (homeowner alarm visibility — should see/hear it from house)",
      "🌀 Inspect for dirty/clogged air filter — replace if needed (~$10, annual replacement); control panel door open for 4-5 minutes to ventilate during check",
      // Pump & breaker confirmation
      "Test pump operation: After breaker reset/check, confirm pump is running (listen/feel for operation)",
      "Confirm double-check: Never leave breakers/powers off normally — ensure back to ON",
      "If breaker trips immediately, note as critical issue (possible short, seized pump, etc.)",
      // NORWECO special
      "For NORWECO or similar entry: Inspect bio-kinetic basket — clean if needed (annual, especially in warm weather)",
    ],
    requiresPhoto: true,
  ...
  },
  {
    ...
    detailedInstructions: [
      ...
      "Clean drip filter",
      "Flush the field after cleaning the drip filter",
