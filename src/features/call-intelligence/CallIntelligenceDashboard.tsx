...
export function CallIntelligenceDashboard() {
  ...
  const agents = agentPerformanceQuery.data?.agents || [];
  const dispositions = dispositionStatsQuery.data?.dispositions || [];
  const totalDispositionCalls = dispositionStatsQuery.data?.total_calls || 0;
  const heatmapData = qualityHeatmapQuery.data?.data || [];
