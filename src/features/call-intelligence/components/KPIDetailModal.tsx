...
export function KPIDetailModal({
      ...
      case "positive":
        filtered = filtered.filter(
          (c) =>
            ...
            (c.sentiment_score && c.sentiment_score > 20),
        );
        break;
      case "neutral":
        filtered = filtered.filter(
    ...
    if (metricId === "escalation-rate" && activeTab === "all") {
      filtered = filtered.filter(
        (c) =>
          ...
          c.escalation_risk === "critical" ||
          c.escalation_risk === "medium",
      );
    }
  ...
  const getSentimentColor = (sentiment?: string, score?: number) => {
    if (sentiment === "positive" || (score && score > 20))
      return "text-green-600 bg-green-50";
    if (sentiment === "negative" || (score && score < -20))
            ...
            <div className="flex gap-2 mt-4">
              {modalConfig.tabs.map((tab) => (
                ...
                >
                  ...
                  {tab === "high-risk" &&
                    `High Risk (${calls.filter((c) => c.escalation_risk === "high").length})`}
                  {tab === "critical" &&
                    `Critical (${calls.filter((c) => c.escalation_risk === "critical").length})`}
                </button>
              ))}
            </div>
          )}
                        ...
                        <td className="py-3 px-4">
                          ...
                          >
                            ...
                            {call.sentiment_score != null &&
                              ` (${call.sentiment_score > 0 ? "+" : ""}${call.sentiment_score.toFixed(0)})`}
                          </span>
                        </td>
                      )}
                      {(metricId === "quality-score" ||
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={cn(
                              "font-semibold",
                              getQualityColor(call.quality_score),
                            )}
                          >
        ...
        {totalPages > 1 && (
          <div className="flex-shrink-0 border-t p-4 flex items-center justify-between bg-bg-subtle">
            <div className="text-sm text-text-secondary">
              ...
              {Math.min(currentPage * pageSize, filteredCalls.length)} of{" "}
