...
export function ReminderDetail() {
    ...
    queryFn: async () => {
      try {
        ...
        return {
          ...
          enabled: true,
          template_id: null,
          custom_message: 'Hi {{customer_name}}, this is a reminder about your appointment tomorrow at {{time}}.',
        };
      }
    },
    enabled: !!id,
  ...
  const toggleMutation = useMutation({
    mutationFn: async () => {
      await apiClient.patch(`/reminders/${id}`, { enabled: !reminder?.enabled });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['reminder', id] });
    },
  ...
  return (
    <div className="p-6">
      {/* Header */}
      <div className="mb-6">
        <Link to="/communications/reminders" className="text-text-muted hover:text-text-primary mb-2 inline-block">
          &larr; Back to Reminders
        </Link>
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-text-primary">{reminder?.name}</h1>
      ...
      <div className="bg-bg-card border border-border rounded-lg p-4 mb-6">
        <div className="flex items-center gap-3">
          <div className={`w-3 h-3 rounded-full ${reminder?.enabled ? 'bg-success' : 'bg-text-muted'}`}></div>
          <span className={`font-medium ${reminder?.enabled ? 'text-success' : 'text-text-muted'}`}>
      ...
      {/* Configuration */}
      <div className="bg-bg-card border border-border rounded-lg p-4 mb-6">
        <h2 className="font-medium text-text-primary mb-4">Trigger Configuration</h2>
        <div className="grid md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm text-text-muted mb-1">Trigger Event</label>
            <select
              disabled={!isEditing}
      ...
      {/* Channels */}
      <div className="bg-bg-card border border-border rounded-lg p-4 mb-6">
        <h2 className="font-medium text-text-primary mb-4">Delivery Channels</h2>
