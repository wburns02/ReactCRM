...
export class SMSService {
  ...
  static canReceiveSMS(
    ...
    if (
      ...
      preferences.opt_out_status === "opted_out"
    ) {
      return false;
    }
  ...
  static buildCommonVariables(data: {
    ...
    if (data.customerFirstName)
      variables.customer_first_name = data.customerFirstName;
    if (data.appointmentDate)
      variables.appointment_date = this.formatDateForSMS(data.appointmentDate);
    if (data.appointmentTime)
      variables.appointment_time = this.formatTimeForSMS(data.appointmentTime);
    if (data.appointmentWindow)
      variables.appointment_window = data.appointmentWindow;
    ...
    if (data.invoiceAmount !== undefined)
      variables.invoice_amount = this.formatCurrencyForSMS(data.invoiceAmount);
    ...
    if (data.workOrderNumber)
      variables.work_order_number = data.workOrderNumber;
   ...
   */
  static validateNotificationRequest(request: SendNotificationRequest): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
