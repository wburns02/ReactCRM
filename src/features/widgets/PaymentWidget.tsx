  ...
  useEffect(() => {
    async function loadInvoice() {
      ...
      try {
        ...
        setErrorMessage(err instanceof Error ? err.message : 'Failed to load invoice');
        setStep('error');
      }
    }
    ...
    return (
      <Card className={cn('max-w-md mx-auto', className)} style={brandStyles}>
        <CardContent className="py-12">
          ...
          <p className="text-center text-text-muted mt-4">Loading payment details...</p>
        </CardContent>
      </Card>
    );
  }
      ...
      <Card className={cn('max-w-md mx-auto', className)} style={brandStyles}>
        <CardContent className="py-12 text-center">
          ...
          <p className="text-text-secondary mb-2">
            Amount paid: <span className="font-semibold">{formatCurrency(paymentAmount * 100)}</span>
          </p>
        ...
        {invoice && (
          <div className="bg-bg-muted rounded-lg p-4 mb-4">
            ...
            {new Date(invoice.dueDate) < new Date() && (
              <Badge variant="warning" className="mt-2">Past Due</Badge>
            )}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            {errors.cardholderName && (
              <p className="text-xs text-danger mt-1">{errors.cardholderName.message}</p>
            )}
          </div>
