...
import type { SubscriptionTier, AISuggestion } from '@/api/types/emailMarketing.ts';
      ...
      {pendingSuggestions.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-text-primary mb-2">No pending suggestions</h3>
            <p className="text-text-secondary mb-4">
              Click "Generate New Suggestions" to have AI analyze your customer base
              and create personalized campaign recommendations.
            </p>
        ...
        <div className="space-y-4">
          {pendingSuggestions.map((suggestion) => {
            const icon = SUGGESTION_ICONS[suggestion.suggestion_type] || SUGGESTION_ICONS.default;
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <div className="text-right">
                          <p className={`text-sm font-medium ${getPriorityColor(suggestion.priority_score)}`}>
                      ...
                      <div className="flex items-center gap-6 text-sm text-text-muted mb-3">
                        {suggestion.estimated_recipients && (
                          <span>
                            <strong>{suggestion.estimated_recipients}</strong> recipients
                          </span>
                        )}
                        {suggestion.estimated_revenue && (
                          <span className="text-success">
                            Est. Revenue: <strong>{formatCurrency(suggestion.estimated_revenue)}</strong>
                          </span>
                        )}
                        {suggestion.target_segment && (
                          <span>
                            Segment: <strong>{suggestion.target_segment}</strong>
                          </span>
                        )}
                        {suggestion.suggested_send_date && (
                          <span>
                            Send: <strong>{formatDate(suggestion.suggested_send_date)}</strong>
                          </span>
                        )}
                      </div>
                      ...
                      {suggestion.ai_rationale && (
                        <div className="bg-bg-muted rounded p-3 mb-3">
                          <p className="text-xs text-text-muted mb-1">AI Rationale:</p>
                          <p className="text-sm text-text-secondary">{suggestion.ai_rationale}</p>
                        </div>
                      )}

                      <div className="flex items-center gap-2">
                        <Button size="sm" onClick={() => handlePreview(suggestion)}>
                          Preview
                        </Button>
                        <Button
                          size="sm"
      ...
      <Dialog open={isPreviewOpen} onClose={() => setIsPreviewOpen(false)}>
        <DialogContent size="lg">
            ...
            {selectedSuggestion && (
              <div className="space-y-4">
                <div className="bg-bg-muted rounded p-3">
                  <p className="text-xs text-text-muted mb-1">Suggested Subject:</p>
                  <p className="font-medium text-text-primary">
                    {selectedSuggestion.suggested_subject || 'No subject generated'}
                  </p>
                </div>

                <div>
                  <p className="text-xs text-text-muted mb-2">Suggested Content:</p>
                  <div className="border border-border rounded p-4 bg-white">
                    {selectedSuggestion.suggested_body ? (
                      <div
                        dangerouslySetInnerHTML={{ __html: selectedSuggestion.suggested_body }}
                      />
                    ) : (
                      <p className="text-text-muted">No content generated. The AI will create content when the campaign is approved.</p>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <p className="font-medium">{selectedSuggestion.target_segment || 'All customers'}</p>
                  </div>
                  <div>
                    <p className="text-text-muted">Estimated Recipients:</p>
                    <p className="font-medium">{selectedSuggestion.estimated_recipients || 'Unknown'}</p>
                  </div>
                  <div>
          ...
          <DialogFooter>
            <Button
              variant="secondary"
              onClick={() => selectedSuggestion && handleDismiss(selectedSuggestion.id)}
              disabled={dismissSuggestion.isPending}
            >
              Dismiss
            </Button>
            <Button
              onClick={() => selectedSuggestion && handleApprove(selectedSuggestion.id)}
              disabled={approveSuggestion.isPending}
            >
              {approveSuggestion.isPending ? 'Approving...' : 'Approve & Create Campaign'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
