    ...
    <Card className={cn('overflow-hidden transition-shadow hover:shadow-md', className)}>
      ...
      <div className="flex items-start gap-3 p-4 pb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <h4 className="font-semibold text-text-primary truncate">{suggestion.title}</h4>
            <Badge variant={confidenceInfo.variant} className="text-xs shrink-0">
              {confidencePercent}%
            </Badge>
          </div>
          <p className="text-sm text-text-secondary line-clamp-2">{suggestion.description}</p>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-xs text-text-muted">{TYPE_LABELS[suggestion.type] || suggestion.type}</span>
      ...
      {isExpanded && (
        <div className="px-4 pb-3 space-y-4 border-t border-border pt-3">
          ...
          {/* Parts Availability */}
          {suggestion.parts_availability && suggestion.parts_availability.length > 0 && (
          ...
          <div className="space-y-1">
            <div className="flex justify-between text-xs">
              ...
              <span className={confidenceInfo.textColor}>{confidenceInfo.label}</span>
            </div>
            <div className="h-2 bg-bg-muted rounded-full overflow-hidden">
              <div
                className={cn('h-full transition-all duration-300', confidenceInfo.color)}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {technician.is_available ? (
            ...
            <Badge variant="default" className="text-xs">Busy</Badge>
          )}
        </div>
        ...
        <div className="flex flex-wrap gap-1 mt-1">
          {technician.skills.slice(0, 4).map((skill) => (
            <span key={skill} className="text-xs px-1.5 py-0.5 bg-bg-muted rounded text-text-secondary">
              {skill}
            </span>
          ))}
          {technician.skills.length > 4 && (
            <span className="text-xs text-text-muted">+{technician.skills.length - 4} more</span>
          )}
        </div>
        ...
        <div className="flex items-center gap-3 mt-2 text-xs text-text-muted">
          ...
          {technician.eta_minutes !== undefined && (
            <span className="text-primary font-medium">ETA: {technician.eta_minutes} min</span>
          )}
          <span>{technician.jobs_today} job{technician.jobs_today !== 1 ? 's' : ''} today</span>
        </div>
      </div>
    </div>
  );
...
function PartsAvailabilitySection({ parts }: { parts: PartsAvailability[] }) {
  const allAvailable = parts.every((p) => p.in_stock && p.quantity_available >= p.quantity_needed);
  const someAvailable = parts.some((p) => p.in_stock);

  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        <h5 className="text-sm font-medium text-text-primary">Parts Required</h5>
        {allAvailable ? (
          <Badge variant="success" className="text-xs">All In Stock</Badge>
        ) : someAvailable ? (
          <Badge variant="warning" className="text-xs">Partial Stock</Badge>
        ) : (
 ...
 */
function EstimatedImpactSection({ impact }: { impact: NonNullable<AIDispatchSuggestion['estimated_impact']> }) {
  return (
    <div className="grid grid-cols-3 gap-3">
      {impact.time_saved_minutes !== undefined && impact.time_saved_minutes > 0 && (
        <div className="text-center p-2 bg-success/10 rounded-lg">
          <div className="text-lg font-semibold text-success">{impact.time_saved_minutes}</div>
          <div className="text-xs text-text-muted">min saved</div>
        </div>
      )}
      {impact.cost_saved !== undefined && impact.cost_saved > 0 && (
        <div className="text-center p-2 bg-success/10 rounded-lg">
          <div className="text-lg font-semibold text-success">${impact.cost_saved}</div>
          <div className="text-xs text-text-muted">cost saved</div>
        </div>
      )}
      {impact.customer_satisfaction && (
        <div className="text-center p-2 bg-primary/10 rounded-lg">
          <div className="text-lg font-semibold text-primary">{impact.customer_satisfaction}</div>
 ...
 */
export function DispatchSuggestionCardSkeleton({ className }: { className?: string }) {
  return (
 ...
 */
export function DispatchSuggestionCardSkeletonList({ count = 3 }: { count?: number }) {
  return (
