...
const PROMPT_TEMPLATES: PromptTemplate[] = [
  {
    label: "Schedule job",
    prompt: "Schedule {technician} for the {customer} job tomorrow at {time}",
    category: "scheduling",
  },
  {
  ...
  },
  {
    ...
    prompt: "Who is available this afternoon?",
    category: "status",
  },
  {
    label: "Reschedule",
    prompt: "Reschedule job {work_order_id} to next available slot",
    category: "scheduling",
  },
  {
    label: "Nearest tech",
    prompt: "Find the nearest available technician to {address}",
    category: "routing",
  },
];
...
export const AICommandInput = memo(function AICommandInput({
  ...
  const handleVoiceResult = useCallback(
    (result: { transcript: string; isFinal: boolean }) => {
      if (result.isFinal && result.transcript) {
        setPrompt((prev) =>
          (prev ? `${prev} ${result.transcript}` : result.transcript).trim(),
        );
      }
    },
    [],
  ...
  const handleSubmit = useCallback(
    (e: React.FormEvent) => {
      ...
      const newHistory = [
        newEntry,
        ...history.filter((h) => h.prompt !== trimmedPrompt),
  ...
  const displayValue =
    prompt + (interimTranscript ? ` ${interimTranscript}` : "");
  ...
  return (
    <div className={cn("relative", className)}>
      <form onSubmit={handleSubmit} className="relative">
        ...
        {voiceError && (
          <div className="mt-2 text-xs text-danger">{voiceError}</div>
        )}
      </form>
      ...
      {showTemplatesDropdown && showTemplates && (
        <div className="absolute z-50 mt-1 w-full bg-bg-card border border-border rounded-lg shadow-lg overflow-hidden">
          ...
          <div className="max-h-64 overflow-y-auto p-2">
            {/* Group by category */}
            {(["scheduling", "routing", "status", "parts"] as const).map(
              (category) => {
                const categoryTemplates = PROMPT_TEMPLATES.filter(
