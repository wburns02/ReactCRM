...
export function ComplianceAlerts({
  ...
  const {
    ...
    isLoading,
    error,
  } = useComplianceDashboard(expiringWithinDays);
  ...
  const hasAlerts =
    (dashboard?.expiring_licenses?.length || 0) > 0 ||
  ...
  return (
    <div className="space-y-4">
      {/* Expiring Licenses */}
      {dashboard?.expiring_licenses &&
        dashboard.expiring_licenses.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2">
                ⚠️ Expiring Licenses
                <Badge variant="warning">
                  {dashboard.expiring_licenses.length}
                </Badge>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                {dashboard.expiring_licenses.map((license) => (
                  <div
                    key={license.id}
                    className="flex items-center justify-between p-2 bg-bg-muted rounded"
                  >
                    <div>
                    ...
                    <div className="text-right">
                      <p
                        className={`text-sm ${license.days_until_expiry !== null && license.days_until_expiry !== undefined && license.days_until_expiry < 0 ? "text-danger" : "text-warning"}`}
                      >
      ...
      {dashboard?.expiring_certifications &&
        dashboard.expiring_certifications.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2">
                🎓 Expiring Certifications
                <Badge variant="warning">
                  {dashboard.expiring_certifications.length}
                </Badge>
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                {dashboard.expiring_certifications.map((cert) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p
                        className={`text-sm ${cert.days_until_expiry !== null && cert.days_until_expiry !== undefined && cert.days_until_expiry < 0 ? "text-danger" : "text-warning"}`}
                      >
      ...
      {/* Overdue Inspections */}
      {dashboard?.overdue_inspections &&
        dashboard.overdue_inspections.length > 0 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-base flex items-center gap-2">
                🚨 Overdue Inspections
                <Badge variant="danger">
                  {dashboard.overdue_inspections.length}
                </Badge>
              </CardTitle>
            </CardHeader>
            <CardContent>
