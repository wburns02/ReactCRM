    ...
    <div className="p-8">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-text-primary">Revenue by Location</h1>
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              value={groupBy}
              onChange={(e) => setGroupBy(e.target.value as 'city' | 'state' | 'zip')}
      ...
      {isLoading && (
        <div className="flex items-center justify-center h-64">
          <div className="text-text-secondary">Loading location revenue data...</div>
        </div>
      )}
      ...
      {error && (
        <div className="p-4 bg-danger/10 border border-danger/20 rounded-lg">
          <p className="text-danger">Failed to load location data. Please try again.</p>
        </div>
      )}
            ...
            {data.locations.length > 0 ? (
              <>
                ...
                <div className="space-y-3 mb-6">
                  {data.locations.map((loc: RevenueByLocationItem, index: number) => {
                      ...
                      <div key={loc.location}>
                        <div className="flex items-center justify-between text-sm mb-1">
                          ...
                          <span className="text-text-muted">
                            {loc.job_count} jobs • {formatCurrency(loc.revenue)}
                          </span>
                        </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-border">
                        <th className="text-left p-2 text-text-muted font-medium">Location</th>
                        <th className="text-right p-2 text-text-muted font-medium">Jobs</th>
                        <th className="text-right p-2 text-text-muted font-medium">Revenue</th>
                        <th className="text-right p-2 text-text-muted font-medium">% of Total</th>
                      </tr>
                    </thead>
                    <tbody>
                      {data.locations.map((loc: RevenueByLocationItem) => (
                        <tr key={loc.location} className="border-b border-border/50">
                          <td className="p-2 font-medium text-text-primary">{loc.location}</td>
                          <td className="p-2 text-right text-text-secondary">{loc.job_count}</td>
                          ...
                          <td className="p-2 text-right text-text-muted">
                            {data.total_revenue > 0
                              ? ((loc.revenue / data.total_revenue) * 100).toFixed(1)
                              : 0}%
                          </td>
                        </tr>
                      ))}
