            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-center p-3 text-text-muted font-medium">
                    Rating
                  </th>
                  <th className="text-center p-3 text-text-muted font-medium">
                    Status
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-3 text-right">
                        <span
                          className={
                            tech.completion_rate >= 90
                              ? "text-success"
                              : tech.completion_rate >= 75
                                ? "text-warning"
                                : "text-danger"
                          }
                        >
          ...
          <div className="mt-4 pt-4 border-t border-border grid grid-cols-5 gap-4">
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-success">
                {formatCurrency(
                  data.reduce((sum, t) => sum + t.revenue_generated, 0),
                )}
              </p>
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-text-primary">
                {data.length > 0
                  ? (
                      data.reduce((sum, t) => sum + t.completion_rate, 0) /
                      data.length
                    ).toFixed(1)
                  : 0}
                %
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-warning">
                {data.length > 0
                  ? (
                      data.reduce((sum, t) => sum + t.jobs_per_day, 0) /
                      data.length
                    ).toFixed(2)
                  : 0}
              </p>
