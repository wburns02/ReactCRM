...
type ContentType = 'ad_copy' | 'email_subject' | 'blog_outline' | 'social_post' | 'landing_page' | 'blog_post';
...
export function AIContentPage() {
  ...
  const isAIConfigured = settings?.integrations?.anthropic?.configured || settings?.integrations?.openai?.configured;
  ...
  const isLoading = generateContent.isPending || generateLandingPage.isPending || generateBlogPost.isPending;
    ...
    switch (selectedType) {
      case 'ad_copy':
        return (
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-text-primary mb-1">Service Focus</label>
              <select
      ...
      case 'email_subject':
        return (
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-text-primary mb-1">Campaign Type</label>
              <select
            ...
            <div>
              <label className="block text-sm font-medium text-text-primary mb-1">Offer/Message</label>
              <input
                type="text"
      ...
      case 'landing_page':
        return (
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-text-primary mb-1">City/Area *</label>
              <input
                type="text"
                value={formData.city}
      ...
      case 'blog_outline':
        return (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-text-primary mb-1">Target Keyword</label>
              <input
                type="text"
                value={formData.keyword}
                onChange={(e) => setFormData({ ...formData, keyword: e.target.value })}
            ...
            {selectedType === 'blog_post' && (
              <div>
                <label className="block text-sm font-medium text-text-primary mb-1">Word Count</label>
                <select
                  value={formData.word_count}
                  onChange={(e) => setFormData({ ...formData, word_count: Number(e.target.value) })}
                  className="w-full px-3 py-2 bg-surface border border-border rounded-md"
                >
            ...
            {blogIdeas?.ideas && (
              <div className="mt-4">
                <p className="text-sm font-medium text-text-primary mb-2">Suggested Topics:</p>
                <div className="flex flex-wrap gap-2">
                  {blogIdeas.ideas.slice(0, 5).map((idea: { topic: string; keyword: string }, index: number) => (
                    <button
                      key={index}
                      className="text-xs px-2 py-1 bg-surface-hover rounded hover:bg-surface-active"
                      onClick={() => setFormData({ ...formData, topic: idea.topic, keyword: idea.keyword })}
                    >
                      {idea.topic}
                    </button>
                  ))}
  ...
  const renderGeneratedContent = () => {
      ...
      return (
        <div className="space-y-4">
          ...
          <div className="p-4 bg-surface-hover rounded-lg">
            ...
            <p className="text-sm"><strong>Title:</strong> {(generatedContent as Record<string, string>).meta_title}</p>
            <p className="text-sm"><strong>Description:</strong> {(generatedContent as Record<string, string>).meta_description}</p>
          </div>
        </div>
      );
    }
    ...
    if (selectedType === 'blog_post') {
      ...
      return (
        <div className="space-y-4">
          <div className="p-4 bg-surface-hover rounded-lg">
            <h3 className="text-lg font-semibold mb-2">{blog.title as string}</h3>
            <p className="text-sm text-text-secondary mb-4">{blog.meta_description as string}</p>
            <div
    ...
    if (typeof generatedContent === 'object') {
      ...
      const headlines = Array.isArray(content.headlines) ? content.headlines as string[] : null;
      const descriptions = Array.isArray(content.descriptions) ? content.descriptions as string[] : null;
      const subjects = Array.isArray(content.subjects) ? content.subjects as string[] : null;
      const posts = Array.isArray(content.posts) ? content.posts as Array<{ text: string; suggested_hashtags?: string[] }> : null;
            ...
            <div>
              ...
              <ul className="space-y-2">
                {headlines.map((h: string, i: number) => (
                  <li key={i} className="p-2 bg-surface-hover rounded flex items-center justify-between">
                    <span>{h}</span>
                    <button
                ...
                {posts.map((post, i: number) => (
                  <div key={i} className="p-3 bg-surface-hover rounded">
                    ...
                    {post.suggested_hashtags && (
                      <p className="text-xs text-primary">{post.suggested_hashtags.join(' ')}</p>
                    )}
                    <button
    ...
    <div className="p-6 space-y-6">
          ...
          <CardContent className="pt-6">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-text-primary">AI Not Configured</h3>
                <p className="text-sm text-text-secondary">
                  Set up your AI provider (Claude or GPT) to enable content generation.
                </p>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle>
              Configure {CONTENT_OPTIONS.find(o => o.id === selectedType)?.label}
            </CardTitle>
          </CardHeader>
          <CardContent>
            {renderForm()}
