...
export function ETADisplay({
  ...
  if (compact) {
    return (
      ...
      >
        <div className={cn('p-2 rounded-full', config.bgColor, config.iconColor)}>
          {config.icon}
        </div>
        ...
        <div className={cn('w-2 h-2 rounded-full animate-pulse', config.pulseColor)} />
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className={cn('px-4 py-3 flex items-center gap-3', config.bgColor)}>
        <div className={cn('p-2 rounded-full bg-white shadow-sm', config.iconColor)}>
          {config.icon}
        </div>
        <div className="flex-1">
      ...
      <div className="bg-white px-4 py-6 text-center">
        {status === 'on_site' ? (
          ...
          <>
            <p className={cn('text-5xl font-bold tracking-tight', config.textColor)}>
              {formattedETA}
            </p>
            {arriving && (
      ...
      {/* Footer stats */}
      {(showArrivalTime || showDistance) && status !== 'on_site' && status !== 'completed' && (
        <div className="bg-gray-50 px-4 py-3 flex justify-center gap-6 border-t border-gray-100">
          {showArrivalTime && arrivalTime && (
            <div className="text-center">
              <p className="text-xs text-gray-500 uppercase tracking-wide">Arrives at</p>
          ...
          {showDistance && distanceText && (
            <div className="text-center">
              ...
              <p className="text-lg font-semibold text-gray-700">{distanceText}</p>
            </div>
          )}
        </div>
      )}
...
export function ETABadge({
  ...
  return (
    ...
    >
      <span
        className={cn('w-1.5 h-1.5 rounded-full animate-pulse', config.pulseColor)}
      />
      {status === 'on_site' ? 'Arrived' : formattedETA}
    </span>
  );
