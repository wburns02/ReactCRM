...
export function AppCard({ app, featured }: AppCardProps) {
  const pricingLabel = {
    ...
    paid: app.pricing.price
      ? `$${app.pricing.price}/${app.pricing.billingPeriod === "yearly" ? "yr" : "mo"}`
      : "Paid",
      ...
      >
        <CardContent className="p-4">
          ...
          <div className="flex items-center justify-between mt-4">
            ...
            {/* Price */}
            <Badge
              variant={app.pricing.type === "free" ? "success" : "secondary"}
              size="sm"
            >
              {pricingLabel}
            </Badge>
          ...
          {app.tags.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-3">
              {app.tags.slice(0, 3).map((tag) => (
                <span
                  key={tag}
                  className="text-xs bg-bg-muted px-2 py-0.5 rounded"
                >
                  {tag}
                </span>
              ))}
