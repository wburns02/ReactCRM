...
export function BenchmarkingPanel() {
  ...
  const [activeTab, setActiveTab] = useState<"benchmarks" | "competitive" | "peers">("benchmarks");

  const { data: benchmarks, isLoading: loadingBenchmarks } = useIndustryBenchmarks();
  const { data: competitive, isLoading: loadingCompetitive } = useCompetitiveAnalysis();
    ...
    <div className="bg-gradient-to-r from-purple-500/10 to-blue-500/10 border border-purple-500/30 rounded-lg p-4">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h4 className="font-medium text-text-primary">AI Benchmarking Analysis</h4>
        </div>
        <button
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto">
        {(["benchmarks", "competitive", "peers"] as const).map((tab) => (
          ...
          >
            {tab === "benchmarks" ? "Industry Benchmarks" :
              tab === "competitive" ? "Competitive Analysis" :
              "Peer Comparison"}
          </button>
        ))}
      </div>
      ...
      {activeTab === "benchmarks" && benchmarks && !loadingBenchmarks && (
        <div className="space-y-3">
          {/* Category groupings */}
          {["financial", "operational", "customer", "efficiency"].map((category) => {
                  ...
                  {categoryBenchmarks.map((benchmark, i) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span className={`text-xs font-medium ${getTrendColor(benchmark.trend)}`}>
                          {benchmark.trend === "above" ? "Above Avg" :
                            benchmark.trend === "below" ? "Below Avg" : "At Avg"}
                        </span>
                      </div>
                      <div className="grid grid-cols-3 gap-2 text-xs mb-2">
                        <div>
                          ...
                          <span className="text-text-primary font-bold">
                            {typeof benchmark.your_value === "number" && benchmark.your_value > 1000
                        ...
                        <div>
                          ...
                          <span className="text-text-secondary">
                            {typeof benchmark.industry_average === "number" && benchmark.industry_average > 1000
                              ? `${(benchmark.industry_average / 1000).toFixed(0)}k`
                              : benchmark.industry_average}
                          </span>
                        </div>
                        <div>
                          <span className="text-text-muted block">Top 10%</span>
                          <span className="text-green-400">
                            {typeof benchmark.top_performers === "number" && benchmark.top_performers > 1000
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-text-muted">{benchmark.percentile}th</span>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {activeTab === "competitive" && competitive && !loadingCompetitive && (
        <div className="space-y-4">
          ...
          <div className="bg-bg-card border border-border rounded-lg p-4 text-center">
            <span className="text-xs text-text-muted block mb-1">Market Position Score</span>
            <span className="text-3xl font-bold text-text-primary">{competitive.overall_score}</span>
            <span className="text-xs text-text-muted">/100</span>
            <div className="mt-2">
              <span className={`text-xs px-2 py-1 rounded border ${getPositionBadge(competitive.market_position)}`}>
                {competitive.market_position.charAt(0).toUpperCase() + competitive.market_position.slice(1)}
              </span>
            </div>
          </div>
              ...
              {competitive.strengths.map((s, i) => (
                <div key={i} className="bg-green-500/10 border border-green-500/30 rounded-lg p-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-green-400">{s.area}</span>
          ...
          <div>
            ...
            <div className="space-y-2">
              {competitive.weaknesses.map((w, i) => (
                <div key={i} className="bg-red-500/10 border border-red-500/30 rounded-lg p-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-red-400">{w.area}</span>
                    <span className={`text-xs px-1.5 py-0.5 rounded ${
                      w.improvement_priority === "high" ? "bg-red-500/20 text-red-400" :
                      w.improvement_priority === "medium" ? "bg-yellow-500/20 text-yellow-400" :
                      "bg-gray-500/20 text-gray-400"
                    }`}>
                      {w.improvement_priority}
                    </span>
                  </div>
                  <p className="text-xs text-text-secondary mt-1">{w.description}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Opportunities */}
          <div>
            <span className="text-xs text-text-muted block mb-2">Growth Opportunities</span>
            <div className="space-y-2">
              {competitive.opportunities.map((o, i) => (
                <div key={i} className="bg-purple-500/10 border border-purple-500/30 rounded-lg p-2">
                  <span className="text-sm font-medium text-purple-400">{o.title}</span>
      ...
      {activeTab === "peers" && peers && !loadingPeers && (
        <div className="space-y-4">
          {/* Rank Card */}
          <div className="bg-bg-card border border-border rounded-lg p-4 text-center">
            <span className="text-xs text-text-muted block mb-1">Your Rank</span>
            <span className="text-3xl font-bold text-primary">#{peers.your_rank}</span>
            <span className="text-xs text-text-muted block">of {peers.peer_count} peers</span>
            <p className="text-xs text-text-secondary mt-2">{peers.peer_group}</p>
          </div>

          {/* Metrics Comparison */}
          <div className="space-y-2">
            {peers.metrics.map((m, i) => {
              const isGood = m.better_if === "higher"
                ...
                <div key={i} className="bg-bg-card border border-border rounded-lg p-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-sm font-bold ${isGood ? "text-green-400" : "text-red-400"}`}>
                      {m.unit === "$" ? `$${m.your_value.toLocaleString()}` :
                        `${m.your_value}${m.unit}`}
                    </span>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div>
                      ...
                      <span className="text-text-secondary block">
                        {m.unit === "$" ? `$${m.peer_average.toLocaleString()}` :
                    ...
                    <div>
                      ...
                      <span className="text-green-400 block">
                        {m.unit === "$" ? `$${m.peer_best.toLocaleString()}` :
                          `${m.peer_best}${m.unit}`}
                      </span>
                    </div>
                  </div>
