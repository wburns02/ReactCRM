...
import { getReferenceDocPreviewUrl } from "@/api/hooks/useReferenceDocs.ts";
import type { ReferenceDoc } from "@/api/hooks/useReferenceDocs.ts";

interface ReferenceDocPreviewModalProps {
  doc: ReferenceDoc;
  isOpen: boolean;
  onClose: () => void;
  onSendEmail: () => void;
...
export function ReferenceDocPreviewModal({
  doc,
  isOpen,
  onClose,
  onSendEmail,
  onDownload,
}: ReferenceDocPreviewModalProps) {
  const [isLoading, setIsLoading] = useState(true);
  const [hasError, setHasError] = useState(false);

  const previewUrl = getReferenceDocPreviewUrl(doc.slug);
