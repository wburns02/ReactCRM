#!/usr/bin/env python3
"""
Tennessee TDEC FileNet Recursive Scraper

Uses recursive search patterns to capture ALL records:
- Search each county with address patterns: 1, 2, 3... A, B, C... etc.
- This ensures we capture all records even if pagination is limited
...
# Search patterns for recursive scraping - addresses commonly start with these
...
async def search_county_pattern(page, county_value, pattern):
    ...
    try:
        ...
        if not street_input:
            ...
            street_input = await page.evaluate('''() => {
                const labels = document.querySelectorAll('label');
                for (const label of labels) {
                    if (label.innerText.toLowerCase().includes('street')) {
            ...
            if street_input:
                street_input = await page.query_selector('input')
...
async def search_county_recursive(page, county_name):
    ...
    # First, get the county value from dropdown
    await page.goto(BASE_URL, wait_until='networkidle', timeout=60000)
    await asyncio.sleep(SHORT_WAIT)
...
async def main():
    ...
    logger.info("=" * 60)
    logger.info("TENNESSEE TDEC RECURSIVE SCRAPER - STARTING")
    logger.info("=" * 60)
    logger.info(f"Counties: {len(TN_COUNTIES)}")
    ...
    async with async_playwright() as p:
        ...
        try:
            for i, county in enumerate(TN_COUNTIES):
                logger.info(f"\n[{i+1}/{len(TN_COUNTIES)}] {county} County")
                ...
                if (i + 1) % 3 == 0:
                    checkpoint = OUTPUT_DIR / f'tn_recursive_checkpoint_{i+1}.json'
