#!/usr/bin/env python3
"""
New Hampshire DES OneStop Septic/Subsurface Scraper

Target: 140,000+ records
Source: NH DES OneStop portal and ArcGIS services
"""
...
OUTPUT_DIR = Path('C:/Users/Will/crm-work/ReactCRM/scrapers/output/new_hampshire')
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# New Hampshire DES ArcGIS endpoints
NH_ENDPOINTS = [
    # NH GRANIT GIS
    ...
    # NH State GIS
    "https://gis.nh.gov/arcgis/rest/services",
]

SEPTIC_KEYWORDS = ['septic', 'sewage', 'wastewater', 'subsurface', 'onsite', 'owts', 'disposal', 'treatment']
...
async def discover_nh_septic():
    """Discover and scrape New Hampshire septic data."""
    logger.info("=" * 60)
    logger.info("NEW HAMPSHIRE DES SEPTIC DISCOVERY")
    logger.info("=" * 60)
    ...
    async with aiohttp.ClientSession() as session:
        for endpoint in NH_ENDPOINTS:
            ...
            # Check services
            for svc in services[:30]:  # Limit display
                name = svc.get('name', '').lower()
                svc_type = svc.get('type', 'FeatureServer')
                logger.info(f"    Service: {svc.get('name')}")

                if any(kw in name for kw in SEPTIC_KEYWORDS) or 'environment' in name or 'des' in name:
                    svc_url = f"{endpoint}/{svc.get('name')}/{svc_type}"

                    layers = await get_layers(session, svc_url)
                    for layer in layers:
                        layer_name = layer.get('name', '').lower()
                        if any(kw in layer_name for kw in SEPTIC_KEYWORDS) or 'permit' in layer_name or 'site' in layer_name:
            ...
            for folder in folders:
                ...
                for svc in folder_services:
                    ...
                    if any(kw in name for kw in SEPTIC_KEYWORDS) or 'des' in name:
                        svc_url = f"{endpoint}/{svc.get('name')}/{svc_type}"
...
async def main():
    logger.info("=" * 60)
    logger.info("NEW HAMPSHIRE DES SEPTIC SCRAPER")
    logger.info("=" * 60)

    records = await discover_nh_septic()

    logger.info("\n" + "=" * 60)
    logger.info("NEW HAMPSHIRE SCRAPER COMPLETE")
    logger.info(f"Total records: {len(records):,}")
    logger.info("=" * 60)

    if records:
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        output_file = OUTPUT_DIR / f'new_hampshire_septic_{timestamp}.json'
