#!/usr/bin/env python3
"""
South Carolina DHEC Environmental Septic Scraper

Target: 200,000+ records
Source: SC DHEC Environmental Applications
"""
...
OUTPUT_DIR = Path('C:/Users/Will/crm-work/ReactCRM/scrapers/output/south_carolina')
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# South Carolina ArcGIS endpoints
SC_ENDPOINTS = [
    "https://services.arcgis.com/EVsTT4nNRCwmHNyb/arcgis/rest/services",
    "https://gis.dhec.sc.gov/arcgis/rest/services",
    "https://maps.scdhec.gov/arcgis/rest/services",
    "https://services2.arcgis.com/XZg2efAw1ksqkCan/arcgis/rest/services",
]

SEPTIC_KEYWORDS = ['septic', 'sewage', 'wastewater', 'onsite', 'owts', 'treatment', 'permit']
...
async def discover_sc_septic():
    """Discover and scrape South Carolina septic data."""
    logger.info("=" * 60)
    logger.info("SOUTH CAROLINA DHEC DISCOVERY")
    logger.info("=" * 60)
    ...
    async with aiohttp.ClientSession() as session:
        for endpoint in SC_ENDPOINTS:
            ...
            for svc in services:
                ...
                if any(kw in name for kw in SEPTIC_KEYWORDS) or 'environment' in name or 'health' in name:
                    ...
                    for layer in layers:
                        ...
                        if any(kw in layer_name for kw in SEPTIC_KEYWORDS) or 'permit' in layer_name:
...
async def main():
    logger.info("=" * 60)
    logger.info("SOUTH CAROLINA DHEC SCRAPER")
    logger.info("=" * 60)

    records = await discover_sc_septic()

    logger.info("\n" + "=" * 60)
    logger.info("SOUTH CAROLINA SCRAPER COMPLETE")
    logger.info(f"Total records: {len(records):,}")
    logger.info("=" * 60)

    if records:
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        output_file = OUTPUT_DIR / f'south_carolina_septic_{timestamp}.json'
